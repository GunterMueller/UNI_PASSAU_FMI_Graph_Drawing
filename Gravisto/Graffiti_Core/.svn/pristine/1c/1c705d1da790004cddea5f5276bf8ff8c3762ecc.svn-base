 ...
 */
public class SCCDecyclingWithoutDeletion extends AbstractAlgorithm implements
        DecyclingAlgorithm {
    /** The algorithm's name */
    private final String ALGORITHM_NAME = "SCCDecycling without edge deletion";
     ...
     * Class constructor.
     */
    public SCCDecyclingWithoutDeletion() {
        checked = false;
    ...
    public void execute() {
        ...
        ts.attach(graph);
        ts.execute();
        for (Edge e : data.getDeletedEdges()) {
            int posSource = e.getSource().getInteger("sugiyama.topo");
            int posTarget = e.getTarget().getInteger("sugiyama.topo");
            if (posSource < posTarget) {
                graph.addEdgeCopy(e, e.getSource(), e.getTarget());
                // System.out.println("nicht umgedreht");
            } else {
                Edge newEdge = graph.addEdgeCopy(e, e.getTarget(), e
                        .getSource());
                data.getReversedEdges().add(newEdge);
                // System.out.println("umgedreht");
            }
        }
        data.setDeletedEdges(new HashSet<Edge>());
