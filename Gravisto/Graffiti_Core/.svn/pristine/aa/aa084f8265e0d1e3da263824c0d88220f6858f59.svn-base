...
/**
 * Slight changes from matzeder's version. This version repulses only not-label
 * nodes from edges.
 * 
 * @author scholz
 */
public class RepulsiveNodeEdgeForce extends AbstractSEForce {
     ...
     */
    public RepulsiveNodeEdgeForce(SELabelingAlgorithmParameters p) {
        super(p);
    }
     ...
     * 
     */
    protected void calcNodeEdgeRepulsiveForces() {

        for (FRNode node : parameters.fRGraph.getNotLabelNodes()) {
                ...
                if (edge.getSource() != node && edge.getTarget() != node
                    ...
                    if (node.isMovable() || edge.getSource().isMovable()
                        ...
                        if (parameters.isCalculationWithNodeSizes) {
                            ...
                            repForce = calcRepForceNodeEdgeWithNodeSizes(node,
                                    edge);
                        } else {
                        ...
                        if (node.isMovable()) {
                            ...
                            GeometricalVector nodeForceVector = node
                                    .getForce(NODE_EDGE_FORCE);

                            // add the repForce GeometricalVector to the
                            // GeometricalVector of node
                            node.setForces(NODE_EDGE_FORCE, GeometricalVector
                                    .add(repForce, nodeForceVector));
                        }
                        if (edge.getSource().isMovable()) {
                            ...
                            // source
                            source.setForces(NODE_EDGE_FORCE, GeometricalVector
                                    .subt(sourceForceVector, repForce));
                        }
                        if (edge.getTarget().isMovable()) {
                            ...
                            // GeometricalVector of
                            // target
                            target.setForces(NODE_EDGE_FORCE, GeometricalVector
                                    .subt(targetForceVector, repForce));
                        }
                    }
                }
        ...
        if (i_nodeIsOnEdge) {
            ...
            if (distanceI_nodeToNodeVector < gamma) {
                ...
                return new GeometricalVector(vec.getX()
                        / parameters.repulsionConstantNodeEdge, vec.getY()
                        / parameters.repulsionConstantNodeEdge);
            }
        }
