...
public class Experiment {
    ...
    private static void runAll() {
        ...
        String[] algorithms = { Config.MST_MAXA, Config.MST_MAXA_QUAD,
                Config.MST_MIN, Config.MST_MIN_QUAD, Config.SE_MST,
                Config.SE_RANDOM };
        ...
        while (fileIt.hasNext()) {
            ...
            for (int i = 0; i < algorithms.length; i++) {
                ...
                while (it.hasNext() && counter < limit) {
                    ...
                    if (!conn.inDatabase(fileName, algorithms[i], levels,
                            width, source.getString("label.label"))) {
                        expBean = exp.execute(source);
                        conn.addResults(expBean);
                    }
                    counter++;
            ...
            /* sqrt(n)sqrt(2) | sqrt(n)sqrt(2) */
            levels = (int) Math.ceil((Math.sqrt(g.getNumberOfNodes()) * Math
                    .sqrt(2)));
            width = levels;
