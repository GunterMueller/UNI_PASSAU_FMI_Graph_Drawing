...
int	find_min_distance_between_pointclusters (int *p1_x, int *p1_y, int n1, int *p2_x, int *p2_y, int n2, int *m1, int *m2)
   	                  	/* 1. Punktecluster, n1 Punkte		*/
	                  	/* 2. Punktecluster, n2 Punkte		*/
   	          		/* Indices der Punkte mit minimalem	*/
				/* Abstand				*/
{
	register int	i = 0, j = 0,
			dist, min_dist = (MAXINT-1);
...
void		write_quoted_text (register FILE *file, register char *text)
{
	register int	i;
...
char	*remove_escape_characters_from_text (register char *text, register int length)
{
	register	int	i,j;
...
char	*remove_control_chars_from_string (char *string)
{
...
int	check_file_is_single (char *filename)
{
	struct	namelist	*file_list;
	int			is_single;
...
char	*file_exists_somewhere (char *filename, char *path)
{
...
char	*get_possible_fileselector_startup_filename (void)
{
	...
	sprintf (filename, "%s/%s", getenv_home, ".graphed_fileselectors");

	return filename;
}


char	*get_existing_fileselector_startup_filename (void)
{
	char	*name;
	char	*getenv_home;
	...
	name = file_exists_somewhere (".graphed_fileselectors", getenv_home);
...
char		*mycalloc (unsigned int n, unsigned int size)
{
	char		*allocated_space;
#ifdef GRAPHED_DEBUG_MALLOC
	static	int	count = 0;
...
void	scale (Scaling scaling, int *x, int *y)
{
	switch (scaling) {
	    case SCALE_16_16 :
		*x = 16;
...
Scaling	size_to_scale (int size)
{
	switch (size) {
	     case 16 :
		return SCALE_16_16;
...
void	constrain_8 (int orgx, int orgy, int *x, int *y)
{
...
void	constrain_to_grid_in_one_dimension (int width, int *x)
{
	int	mod_width;
...
void	constrain_to_grid (int width, int *x, int *y)
{
...
void	calculate_subwindow_position (int subwindow_width, int subwindow_height, int selection_x, int selection_y, int selection_width, int selection_height, int *x, int *y)
{
