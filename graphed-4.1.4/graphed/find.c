...
int	point_is_near_edgeline (register Edgeline el, int point_x, int point_y, int gap_x, int gap_y);
...
Picklist	picker (int x, int y, Pick_mode mode)
{
...
Picklist	xpicker (int x, int y, Pick_mode mode)
{
	Picklist	pl_head = empty_picklist,
			pl      = empty_picklist;
...
Node		node_finder (int x, int y, Find_which which)
{
...
Edge		edge_finder (int x, int y, Find_which which)
{
...
Node	find_node_containing_point (int x, int y)
{
	Node	node;
...
Picked_point_of_node	find_picked_point_of_node (Node node, int x, int y)
{
...
Picked_point_of_edgeline	find_picked_point_of_edgeline (Edgeline el, int x, int y)
{
...
Picklist	new_picklist (What_is_picked what, ...)
{
	Picklist	new_pl;
	Which_is_picked	which;
	va_list		args;
...
Picklist	add_to_picklist (Picklist pl, What_is_picked what, ...)
{
	Picklist	new_pl;
	Which_is_picked	which;
	va_list		args;
...
Picklist	remove_from_picklist (Picklist pl)
{
	if (pl == empty_picklist)
...
void		free_picklist (Picklist pl)
{
	Picklist	next;
...
Picklist	picklist_contains_object (Picklist picklist, Picklist picked_object)
{
	Picklist	pl;
	int		contains = FALSE;
...
Picklist	remove_left_side_of_productions_from_picklist (Picklist picklist)
{
...
void		mark_picked_object (Picklist pl)
{
	Group	g;
...
void		unmark_picked_object (Picklist pl)
{
	Group	g;
	Group	edited_group;
	Node	edited_node;
	...
	switch (pl->what) {
		...
		if (pl->which.node != edited_node &&
		    !contains_group_node (edited_group, pl->which.node))
		...
		for_group (pl->which.group, g) {
		    if (g->node != edited_node &&
		        !contains_group_node (edited_group, g->node))
...
Rect		compute_bounding_rect_of_picklist (Picklist pl)
{
