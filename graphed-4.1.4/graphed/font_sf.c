...
static	char	*derive_font_id_from_filename (char *filename);
...
static	void delete_current_font(void)
{
	if ( what_font == NODE ) {
...
static	void close_font_subframe(void)
{
	fls_close( font_fileselector );
	unlock_user_interface();
}

static 	void	local_insert_font(char *dir, char *file)
{
...
static	void make_font_panel_items(Panel panel)
{
...
void	init_font_fileselector (void)
{
	if( first ) {
		first = FALSE;
...
void		show_font_edit_subframe (Node_or_edge node_or_edge)
{
	lock_user_interface ();
...
void	write_font_fileselector(FILE *file)
{
	init_font_fileselector ();
	if( font_fileselector != NULL ) {
