...
Evp_result		create_mode_event_proc (Xv_Window window, Event *event, Message_to_event_proc message)
{
	int			x,y;
	...
	if (message == EVP_SHUTDOWN) {
		if (ui_state.making_node || ui_state.making_edge ||
	...
	switch (event_id(event)) {
		...
		if (ui_state.making_edge) {
		
			bell ();
			return EVP_ERROR;
			
		} else if (/* !ui_state.making_edge && */ !ui_state.making_node) {
		...
		if (event_is_down(event) && !ui_state.making_edge) {
		...
		} else if (event_is_down(event) && ui_state.making_edge) {
		
			/* Neuer Punkt in der temporary_edgeline	*/
			return drag_edge_proc (window, event, &drag_edge_info);
						
		} else if (event_is_up(event) && ui_state.making_edge) {
		...
		} else /* event_is_up(event) && !ui_state.making_edge	*/ {
		...
		if (ui_state.making_edge && el != (Edgeline)NULL)
			return drag_edge_proc (window, event, &drag_edge_info);
		else if (ui_state.making_node)
