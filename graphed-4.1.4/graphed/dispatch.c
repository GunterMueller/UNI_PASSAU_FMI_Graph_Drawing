...
extern void set_gragra_type (Graph graph, Gragra_type type);
extern void get_buffer_center (int buffer, int *x, int *y);
extern void show_graph_subframe (Graph graph);
extern void show_gragra_subframe (void);
extern void center_buffer_around (int buffer, int x, int y);
extern void set_working_area_canvas (Canvas canvas);
extern int  any_graph_has_changed (void);
extern void show_about_subframe(void);
...
static	int	nothing_edited (void);
static	void	set_node_defaults (Node_defaults_subframe_info *info);
static	void	set_edge_defaults (Edge_defaults_subframe_info *info);
...
Command_flags	commands[NUMBER_OF_WORKING_AREA_MENU_ACTIONS];



void		set_command_flags (User_action command, unsigned flags)
{
	if (flags & MODIFIES_GRAPH_STRUCTURE)
		flags |= MODIFIES_GRAPH;
	if (flags & MODIFIES_GRAPH)
		flags |= MODIFIES_SOMETHING;

	commands[command].modified = flags;
}

unsigned	get_command_flags (User_action command)
{
	return	commands[command].modified;
}
...
void	init_command_flags(void)
{
	...
	/* A *very* lazy initialisation */
	for (i=0; i<NUMBER_OF_WORKING_AREA_MENU_ACTIONS; i++) {
		set_command_flags(i,MODIFIES_NOTHING);
	}

	set_command_flags (CREATE_GRAPH, MODIFIES_GRAPH_STRUCTURE);
	set_command_flags (CREATE_DIRECTED_GRAPH,MODIFIES_GRAPH_STRUCTURE);
	set_command_flags (CREATE_UNDIRECTED_GRAPH,MODIFIES_GRAPH_STRUCTURE);

	set_command_flags (CREATE_PRODUCTION, MODIFIES_GRAPH_STRUCTURE);
	set_command_flags (CREATE_DIRECTED_PRODUCTION, MODIFIES_GRAPH_STRUCTURE);
	set_command_flags (CREATE_UNDIRECTED_PRODUCTION, MODIFIES_GRAPH_STRUCTURE);

	set_command_flags (CREATE_EMBEDDING, MODIFIES_GRAPH_STRUCTURE);
	set_command_flags (CREATE_DIRECTED_EMBEDDING, MODIFIES_GRAPH_STRUCTURE);
	set_command_flags (CREATE_UNDIRECTED_EMBEDDING, MODIFIES_GRAPH_STRUCTURE);

	set_command_flags (CREATE_NODE, MODIFIES_GRAPH_STRUCTURE);
	set_command_flags (CREATE_EDGE, MODIFIES_GRAPH_STRUCTURE);

	set_command_flags (EDIT_GRAPH_OF_SELECTION, MODIFIES_GRAPH_STRUCTURE);
	set_command_flags (GET_SELECTION, MODIFIES_GRAPH_STRUCTURE);
	set_command_flags (GET_AS_GRAPH, MODIFIES_GRAPH_STRUCTURE);
	set_command_flags (SPLIT_EDGE, MODIFIES_GRAPH_STRUCTURE);
	set_command_flags (SPLIT_SELECTION, MODIFIES_GRAPH_STRUCTURE);
	set_command_flags (MERGE_SELECTION, MODIFIES_GRAPH_STRUCTURE);
	set_command_flags (REVERSE_EDGE, MODIFIES_GRAPH_STRUCTURE);

	set_command_flags (DELETE_SELECTION, MODIFIES_GRAPH_STRUCTURE);
	set_command_flags (DELETE_ALL, MODIFIES_GRAPH_STRUCTURE);
	set_command_flags (LOAD_BY_SUBFRAME, MODIFIES_GRAPH_STRUCTURE);
	set_command_flags (LOAD_AGAIN, MODIFIES_GRAPH_STRUCTURE);

	set_command_flags (SWAP_SELECTED_GRAPH_DIRECTEDNESS, MODIFIES_GRAPH_STRUCTURE);
	set_command_flags (SET_GRAPH_DIRECTEDNESS, MODIFIES_GRAPH_STRUCTURE);
	set_command_flags (REVERSE_EDGE, MODIFIES_GRAPH_STRUCTURE);

	set_command_flags (EDIT_SELECTION, MODIFIES_GRAPH);
	set_command_flags (LABEL_SELECTION, MODIFIES_GRAPH);
	set_command_flags (MOVE_SELECTION, MODIFIES_GRAPH);
	set_command_flags (RESIZE_SELECTION, MODIFIES_GRAPH);
	set_command_flags (MOVE_EDGE, MODIFIES_GRAPH);
	set_command_flags (COMPRIME_EDGE, MODIFIES_GRAPH);
	set_command_flags (ZOOM_SELECTION_UP, MODIFIES_GRAPH);
	set_command_flags (ZOOM_SELECTION_DOWN, MODIFIES_GRAPH);

	set_command_flags (APPLY_CURRENT_PRODUCTION, MODIFIES_GRAPH_STRUCTURE);
	set_command_flags (APPLY_PRODUCTION, MODIFIES_GRAPH_STRUCTURE);

	set_command_flags (PRETTY_PRINT_PRODUCTION, MODIFIES_GRAPH);
	set_command_flags (PRETTY_PRINT_CURRENT_PRODUCTION, MODIFIES_GRAPH);
	set_command_flags (PRETTY_PRINT_ALL_PRODUCTIONS, MODIFIES_GRAPH);
	set_command_flags (CENTER_SELECTION, MODIFIES_GRAPH);
}
...
char	*dispatch_user_action (User_action action, ...)
{
	...
	if (get_disable_all_commands()) {
		bell();
		return NULL;
	}

	if ((get_disable_all_modifying_commands() != NULL) &&
            (get_command_flags(action) & MODIFIES_GRAPH) ) {
		error ("All modifications are currently disabled.\n");
		return NULL;
	}

	if ((get_disable_all_structure_modifying_commands() != NULL) &&
            (get_command_flags(action) & MODIFIES_GRAPH_STRUCTURE) ) {
		error ("Graph structure modifications are currently disabled.\n");
		return NULL;
	}
	...
	switch (action) {
		...
		if (nothing_edited() && remove_user_event_proc()) {
			set_graphed_mode (GRAPHED_MODE_CREATE_MODE);
	    ...
	    case EDIT_MODE :
		if (nothing_edited() && remove_user_event_proc()) {
			set_graphed_mode (GRAPHED_MODE_EDIT_MODE);
			set_user_event_proc (edit_mode_event_proc);
		} else
			bell ();
		break;
	    ...
	    case _CREATE_GRAPH :
		{
			...
			dispatcher_result = (char*)last.graph;
		}
		break;
		...
		if (nothing_edited () && remove_user_event_proc()) {
			...
			notice_prompt (base_frame, NULL,		/*fisprompt*/
				NOTICE_MESSAGE_STRINGS,	"Creating graphs can only be done in create mode.", NULL,
		...
		if (nothing_edited() && remove_user_event_proc ()) {
			...
			if (menu_called_from == MENU_CALLED_FROM_CANVAS || menu_called_from == MENU_CALLED_FROM_MENUBAR) {
				buffer = wac_buffer;
			} else {
				buffer = (int)dispatch_user_action (CREATE_BUFFER);
			}
			...
			notice_prompt (base_frame, NULL,		/*fisprompt*/
				NOTICE_MESSAGE_STRINGS,	"Creating productions can only be done in create mode.", NULL,
				NOTICE_BUTTON_YES,	"Ok",
				NULL);
		}
		break;
		...
		if (something_picked) {
			...
			switch (picked_object->what) {
				...
				if (get_graphed_group_labelling_operation () == NODE) {
				    for_group (group, g) {
		...
		} else if (buffers[wac_buffer].graphs == empty_graph) {
			error ("no graph\n");
		} else {
			error ("select a graph\n");
		}
		break;
	...
	(center_of_group_y +  (int)((double)((y) - center_of_group_y) * sy) )
				...
				for_group (group,g) {
					...
					if (contains_group_node (group, edge->target)) {
						...
						if (
						    !(edge->source->node_edge_interface == NO_NODE_EDGE_INTERFACE &&
						      el == edge->line) &&
						    !(edge->target->node_edge_interface == NO_NODE_EDGE_INTERFACE &&
						      el == edge->line->pre)
						    ) {
		...
		if (nothing_edited ()) {
			...
			if (graph != empty_graph) {
				...
				if (group_of_graph != empty_group) {
					group_set (group_of_graph,
						RESTORE_IT,
						0);
				}
		...
		if (something_picked) {
			...
			switch (picked_object->what) {
				...
				if (!put_whole_graph) {
					...
					(void) copy_graph (paste_buffer, picked_object->which.edge->source->graph);
				}
				break;
			    case GROUP_PICKED :
				if (!put_whole_graph) {
...
/*
		if (ui_state.user_event_proc == create_mode_event_proc) {
...
*/
	    ...
	    case DELETE_ALL:
		if (nothing_edited () &&
		    /*ui_state.user_event_proc abmelden, da sonst "dangling pointer"	*/
		    /* entstehen koennen !					*/
		...
		if (ui_state.user_event_proc == create_mode_event_proc) {
			if (!remove_user_event_proc()) {
				notice_prompt (base_frame, NULL,		/*fisprompt*/
					NOTICE_MESSAGE_STRINGS,	"Compiling can only be done in edit mode.", NULL,
					NOTICE_BUTTON_YES,	"Ok",
					NULL);
				break;
			}
	    ...
	    case COMPILE_ALL_PRODUCTIONS :
		{
			...
			if (ui_state.user_event_proc == create_mode_event_proc && !remove_user_event_proc()) {

				notice_prompt (base_frame, NULL,		/*fisprompt*/
					NOTICE_MESSAGE_STRINGS,	"Compiling can only be done in edit mode.", NULL,
					NOTICE_BUTTON_YES,	"Ok",
					NULL);
				dispatcher_result = (char *)FALSE;
				...
				dispatcher_result = (char *)no_errors;
			}

			rebuild_gragra_productions_submenu();
		}
		break;
	    ...
	    case SET_CURRENT_PRODUCTION :
		{
			...
			if (something_picked && prod != empty_graph && prod->is_production) {
				...
				current_production = empty_graph;
				error ("Graph is not a production");
			}
		}
		break;	
		...
		{
			Gragra_apply_form	apply_form = va_arg (args, Gragra_apply_form);
			Group			derived_right_side;
					
			dispatch_user_action (UNSELECT);
			if (last.node != empty_node && last.node == apply_form->apply_on) {
				last.node = empty_node;
			}
			if (last.edge != empty_edge && (last.edge->source == apply_form->apply_on || last.edge->target == apply_form->apply_on)) {
				last.edge = empty_edge;
			}
			derived_right_side = apply_production (apply_form->production, apply_form->apply_on);
			/* graph_set (apply_form->apply_on, RESTORE_IT, NULL); */
		...
		if (current_production != empty_graph &&
		    something_picked && (picked_object->what == NODE_PICKED ||
		                         (picked_object->what == GROUP_PICKED && picked_object->which.group == picked_object->which.group->suc))) {
			...
			if (picked_object->what == NODE_PICKED) {
				...
				node = picked_object->which.group->node;
			}
			...
			if (compile_production_error_message != NULL) {
				/* nothing */
			} else if (node->graph == current_production) {
				notice_prompt (base_frame, NULL,		/*fisprompt*/
					NOTICE_MESSAGE_STRINGS,	"You can't apply a production on itself.", NULL,
					NOTICE_BUTTON_YES,	"Ok",
					NULL);
			} else if (current_production->directed != node->graph->directed) {
				notice_prompt (base_frame, NULL,		/*fisprompt*/
					NOTICE_MESSAGE_STRINGS,	"Production and node have different directeness.", NULL,
					NOTICE_BUTTON_YES,	"Ok",
					NULL);
			} else if (embed_match_node (current_production->gra.gra.nce1.left_side->node, node)) {
				struct gragra_apply_form apply_form;				
				apply_form.apply_on = node;
				apply_form.production = current_production;
	    ...
	    case RANDOM_APPLY_PRODUCTION :
		{
			Graph	picked_graph;
			Node	node;
			Slist	productions_to_apply = empty_slist;
			Gragra_apply_form	apply_form;
			int			number_of_productions_to_apply, random_form, i;

			picked_graph = get_picked_graph ();			
			dispatch_user_action (COMPILE_ALL_PRODUCTIONS);
			...
			} else if (picked_graph != empty_graph) {

			    /* Create a list of productions tht can be applied on a node of the graph */
			    for_nodes (picked_graph, node) {
				for (buffer=N_PASTE_BUFFERS; buffer<N_BUFFERS; buffer++) {
				    for_all_graphs (buffer, graph) {
					if (graph->is_production &&
					    graph != picked_graph &&
					    graph->directed == picked_graph->directed &&
					    embed_match_node (graph->gra.gra.nce1.left_side->node, node)) {
						apply_form = (Gragra_apply_form)malloc (sizeof (struct gragra_apply_form));
						apply_form->apply_on = node;
						apply_form->production = graph;
						productions_to_apply = add_to_slist (productions_to_apply , make_attr (ATTR_DATA, (char *)apply_form));
					}
				    } end_for_all_graphs (buffer, graph);
				}
			    } end_for_nodes (picked_graph, node);

			    /* Randomly select a production / node to apply */
			    number_of_productions_to_apply = size_of_slist (productions_to_apply);
			    if (number_of_productions_to_apply > 0) {
				random_form = (random() % number_of_productions_to_apply);
				for (i=0; i<random_form; i++) {
				    productions_to_apply = productions_to_apply->suc;
				}
				dispatch_user_action (APPLY_PRODUCTION,
					attr_data_of_type (productions_to_apply, Gragra_apply_form));
			    } else {
				message ("No production to apply\n");
			    }
			}
		}
		break;
		...
		{
			Graph	graph;
			int 	n_nodes = 0, n_edges = 0, n_graphs = 0;
			
			message ("buffer \"%s\" :\n",
				iif (buffers[wac_buffer].filename != NULL, buffers[wac_buffer].filename, ""));
			for_all_graphs (wac_buffer, graph) {
				...
				message ("graph %d \"%s\" :  %d nodes,  %d edges\n",
					n_graphs,
					iif (graph->label != NULL, graph->label, ""), n_nodes,n_edges);
	    ...
	    case ALL_STATISTICS :
		{
			...
			int 	n_nodes_this_buffer = 0, n_edges_this_buffer = 0, n_graphs_this_buffer = 0;
			
			for (buffer=N_PASTE_BUFFERS; buffer<N_BUFFERS; buffer++)
			    if (buffers[buffer].graphs != empty_graph && buffers[buffer].used) {

				message ("buffer %d \"%s\" :\n",
					buffer,
					iif (buffers[buffer].filename != NULL, buffers[buffer].filename, ""));

				n_graphs_this_buffer = 0;
				n_nodes_this_buffer = 0;
				n_edges_this_buffer = 0;
				...
				for_all_graphs (buffer, graph) {
					...
					n_nodes_this_buffer += n_nodes;
					n_edges_this_buffer += n_edges;
					n_graphs_this_buffer ++;
				...
				message ("summary buffer %d :\n  %d graphs,  %d nodes,  %d edges\n",
					buffer,
					n_graphs_this_buffer,
					n_nodes_this_buffer,
					n_edges_this_buffer);
		...
		if (test_user_interface_locked() || nothing_edited() == FALSE) {
			bell ();
		} else if (!buffers[wac_buffer].changed ||
			   NOTICE_YES == notice_prompt(base_frame, NULL,		/*fisprompt*/
			...
			if (menu_called_from == MENU_CALLED_FROM_NOWHERE) {
				buffer = (int)dispatch_user_action (CREATE_BUFFER);
				if (buffer != -1) {
		...
		if (menu_called_from == MENU_CALLED_FROM_NOWHERE) {
			...
			bell ();
		} else if (buffer_get_filename (wac_buffer) == NULL || !strcmp(buffer_get_filename(wac_buffer),"") || (buffer_get_filename(wac_buffer))[0] == '-' ) {
	    ...
	    case LOAD_AGAIN_SILENT :
		dispatch_user_action (BASIC_LOAD, wac_buffer, buffers[wac_buffer].filename);
		break;
		...
		strcpy (filename, va_arg(args, char *));
		load_from_file (buffer, filename, LOAD_ANY_FILE);
		break;
		...
		if (test_user_interface_locked()) {
			/* skip */;
		} else if (menu_called_from == MENU_CALLED_FROM_NOWHERE) {
			notice_prompt (base_frame, NULL,		/*fisprompt*/
		...
		if (menu_called_from == MENU_CALLED_FROM_NOWHERE) {
			notice_prompt (base_frame, NULL,		/*fisprompt*/
				...
				NOTICE_BUTTON_YES,	"Ok",
				NULL);
		} else if (buffer_get_filename(wac_buffer) == NULL || !strcmp(buffer_get_filename(wac_buffer),"")) {
			dispatch_user_action (STORE_BY_SUBFRAME);
		} else {
			dispatch_user_action (BASIC_STORE, wac_buffer, buffer_get_filename(wac_buffer));
		}
		break;

	
	    case STORE_SILENT_TO_FILE :
		dispatch_user_action (BASIC_STORE, wac_buffer, va_arg(args, char *));
		break;
		...
		if (buffers[buffer].graphs != empty_graph) {
			if (buffers[buffer].graphs->suc != buffers[buffer].graphs) {
				store_graphs (filename);
			} else {
				store_graph (buffers[buffer].graphs, filename);
			}
		}
		break;
		...
		dispatch_user_action (SCALE_EDGELABELSIZE, 1024, 1024);
		break;
		...
		set_graphed_constrained(TRUE);
		break;

	    case RESET_CONSTRAINED :
		set_graphed_constrained(FALSE);
		break;

	    case TOGGLE_CONSTRAINED :
		if (get_graphed_constrained())
		...
		set_graphed_group_labelling_operation (NODE);
		break;

	    case GROUP_LABELLING_OPERATION_GOES_TO_EDGE :
		set_graphed_group_labelling_operation (EDGE);
		break;
