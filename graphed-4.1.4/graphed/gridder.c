...
static	void	notify_gridder_cycles (Panel_item item, int value, Event *event);

void   compute_largest_node_sizes_in_graph (Graph graph, int *largest_node_width, int *largest_node_height);
...
static	void	notify_gridder_cycles (Panel_item item, int value, Event *event)
{
	...
	gridder_set (gridder, value, atoi((char *)xv_get(gridder->text, PANEL_VALUE)));
}
...
void		gridder_set (Gridder gridder, Gridder_choices value, int size)
{
	...
	switch (value) {
		...
		xv_set(gridder->text,
			XV_SHOW,	FALSE,
			PANEL_VALUE,	int_to_ascii (recompute_gridder_size (gridder,(Gridder_choices)0 ,(Gridder_width_or_height)0)),
			NULL);
		break;
	    case GRIDDER_DISTANCE_OTHER :
		xv_set(gridder->text,
...
int			recompute_gridder_size (Gridder gridder, Gridder_choices choice, Gridder_width_or_height kind)
{
	...
	switch (choice) {
		...
		if (gridder != (Gridder)NULL) {
			largest_node_width  = atoi((char *)xv_get(gridder->text, PANEL_VALUE));
			largest_node_height = atoi((char *)xv_get(gridder->text, PANEL_VALUE));
		} else {
			compute_largest_node_sizes_in_graph (
				get_picked_or_only_existent_graph(),
				&largest_node_width,
...
int	gridder_get_size (Gridder gridder)
{
	return recompute_gridder_size (gridder,(Gridder_choices)0 ,(Gridder_width_or_height)0);
}
...
void	compute_largest_node_sizes_in_selection (int *largest_node_width, int *largest_node_height)
{
	Picklist	selection;
	Group		g;
