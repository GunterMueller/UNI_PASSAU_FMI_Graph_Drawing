...
Graph	create_graph_internal (int buffer, char *sgraph)
{
	Graph		new_graph;
	...
	new_graph->derivation_history = (struct sgraph *)NULL;
	...
	new_graph->file_attrs   = NULL;

	if (sgraph == NULL) {
		attatch_sgraph (new_graph);
	} else {
		new_graph->sgraph_graph = sgraph;
	}

#ifdef LP_LAYOUT
	/* Macht auch gra.gra.nce1.lp_nce1_gragra */
	init_lp_graph( new_graph );
#endif
...
Graph	create_graph (int buffer)
{
	return create_graph_internal (buffer, NULL);
}
...
void	delete_graph (Graph graph)
{
	...
	if (graph->derivation_history != NULL) {
		remove_graph (graph->derivation_history);
	}
	...
	free_file_attributes (graph->file_attrs);

#ifdef LP_LAYOUT
	clear_lp_graph( graph );
#endif
...
Graph	copy_graph (int buffer, Graph graph)
{
	Graph	new_graph;
	Node	node;
	Edge	edge;
...
Group	copy_graph_to_graph (Graph to_graph, Graph from_graph)
{
...
void	set_graph_directedness (Graph graph, int directed)
{
	graph->directed = directed;
	if (graph->sgraph_graph != NULL) {
		set_graph_directed ((Sgraph)graph->sgraph_graph, directed);
	}
}



void	set_graph_label (Graph graph, char *label)
{
	if (graph->label != NULL)
		myfree (graph->label);
	graph->label = label;
...
void	erase_and_delete_graph (Graph graph)
{
	delete_graph (graph);
...
Graph	create_production (int buffer)
{
...
void		set_gragra_type (Graph graph, Gragra_type type)
{
	graph->gra.type = type;
...
void		set_graph_file_attributes (Graph graph, File_attributes attrs)
{
	graph->file_attrs = attrs;
}


File_attributes	get_graph_file_attributes (Graph graph)
{
	return graph->file_attrs;
}
...
void	set_graph_has_changed (Graph graph)
{
	if (graph != NULL) {
...
void	reset_graph_has_changed (Graph graph)
{
	int	buffer,
		all_unchanged;
	Graph	g;
...
void	reset_buffer_has_changed (int buffer)
{
	if (buffers[buffer].changed) {
...
int	graph_has_changed (Graph graph)
{
	return graph->changed;
}


int	any_graph_has_changed (void)
{
	int	i;
...
Rect	compute_rect_around_graph (Graph graph)
{
	Rect	rect;
	Node	node;
	Edge	edge;
...
Rect	compute_rect_around_graphs (int buffer)
{
	Graph	graph;
...
void	create_graph_attributes (Graph graph, char	*(*create_graph_attr_proc)(),
...
void	push_graph_attributes   (Graph graph)
{
	Node		node;
	Edge		edge;
...
void	pop_graph_attributes    (Graph graph, void	(*destroy_graph_attr_proc)(),
