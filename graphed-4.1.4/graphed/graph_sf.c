...
static	void	hide_graph_subframe           (Frame frame);
/*static void	notify_graph_subframe_buttons (Panel_item item, Event *event);*/
static	void	notify_graph_subframe_cycles  (Panel_item item, int value, Event *event);
...
void	create_graph_subframe (Graph graph)
{
	int	row_count = 0;

	graph_subframe = (Frame)xv_create(base_frame, FRAME_CMD,
		FRAME_LABEL,		"Graph Editor",
	...
	if (graph->is_production) {
		row_count += 1;
		graph_subframe_gragra_type = xv_create(graph_subframe_panel, PANEL_CHOICE,
...
void	show_graph_subframe (Graph graph)
{
	lock_user_interface ();
	
	if (showing_graph_subframe) hide_graph_subframe ((Frame)0);
...
Graph get_currently_edited_graph (void)
{
