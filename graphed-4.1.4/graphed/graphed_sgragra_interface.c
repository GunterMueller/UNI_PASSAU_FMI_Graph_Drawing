...
- Mehrfachaufruf von init_sgragra_from_graphed_gragra ueberpruefen (sgragra-
  pointer in graphed werden ueberschrieben).
- Pruefen, ob sgragra_node in Graphedknoten schon von Sgraph belegt (siehe oben).
...
/*
static
int	test_wac_has_a_production (int canvas_buffer_number)
	/ *
...
static
void	clear_info (Sgragra_proc_info info)
...
static
Sgraph	convert_group_to_sgraph(Sgragra sgragra, Group group)
...
side of the production) the pointer sgragra_node is initialized.
...
*/
{
	...
	for_group(group, g)
	{
		Snode sn;
		Slist *alphabet;
		sn = make_node_with_number(sgraph,
			make_attr(ATTR_DATA, NULL),
			g->node->nr);
		set_node_xy (sn, g->node->x, g->node->y);
		sn->graphed = (char *) g->node;
		g->node->sgragra_node = (char *) sn;
		alphabet = node_is_terminal(
			g->node) ? &(sgragra->tv) : &(sgragra->nv);
	...
	for_group(group, g) {	
		Edge e;
		for_edge_sourcelist(g->node,e) if (contains_group_node(group, e->target)) {
			Sedge se = make_edge((Snode)g->node->sgragra_node,
				(Snode)e->target->sgragra_node,
				make_attr(ATTR_DATA, NULL));
...
static
void	convert_edge_to_embedding(Edge edge, int direction, Sprod sprod)
	...
	*/
	{
		...
		sembed = make_sembed(sprod, (Snode) right_side->sgragra_node);
...
static
void	convert_nce1_embeddings(Sprod sprod, Embedding embedding, int direction)
	...
	the pointer sgragra_mode must have been initialized.	
	*/
	{
...
static
void	convert_embeddings(Sprod sprod, Graph graph)
...
static
int	append_graphed_prod_to_sgragra(Sgragra sgragra, Graph graph)
...
{	
	...
	if (graph->gra.gra.nce1.right_side != empty_group) {
		sprod->right	= convert_group_to_sgraph (sgragra, graph->gra.gra.nce1.right_side);
	} else {
		sprod->right = empty_sgraph;
	}

	sprod->graphed_graph = (char *)graph;
	sprod->graphed_left  = (char *)graph->gra.gra.nce1.left_side->node;
...
static
void	convert_edge_to_global_embedding(Edge edge, int direction, Sgragra sgragra)
...
static
void	convert_nce1_global_embeddings(Sgragra sgragra, Embedding embedding, int direction)
...
static
void	append_global_graphed_embeddings_to_sgragra (Sgragra sgragra, Graph graph)
{
...
static
void	append_graphed_gragra_to_sgragra (Sgragra_proc_info info, int canvas_buffer_number, int autocompile)
...
{	
	...
	{	
		if (graph->is_production)
		{
			...
			{
				if (!append_graphed_prod_to_sgragra(info->sgragra, graph /*, autocompile */)) /* wieso 3 parameter? */
				{
					remove_sgragra(info->sgragra);
					clear_info(info);
					return;
...
static
void	clean_sgragra(Sgragra sgragra)
	...
	*/
{
	...
	/* Restore NULL in all node->sgragra_node and snode->graphed pointers */
	for_all_sprods(sgragra, sprod) if (sprod->right != NULL) {
		for_all_nodes(sprod->right, snode) if (snode->graphed != NULL){
			((Node)snode->graphed)->sgragra_node = NULL;
			snode->graphed = NULL;
...
Sgragra_proc_info 	init_sgragra_from_graphed_gragra(Sgragra_create_mode create_mode, int auto_compile)
{
	...
	switch (create_mode) {
		...
		if (current_production == empty_graph) {
			...
			append_graphed_gragra_to_sgragra(info,
				current_production->buffer,
				auto_compile);
		}
		break;
...
void	exit_sgragra_from_graphed_gragra(Sgragra_proc_info info)
{
