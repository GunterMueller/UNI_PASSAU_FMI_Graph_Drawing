...
extern void install_fontlist_in_group_subframe (char **list);
extern void install_fontlist_in_node_defaults_subframe (char **list);
extern void install_fontlist_in_edge_defaults_subframe (char **list);
extern void install_current_nodefont_in_node_subframe (void);
extern void install_current_edgefont_in_edge_subframe (void);
...
/************************************************************************/
/*									*/
/*	int	insert_font (name, font_id, insert_position)		*/
/*	int	add_font    (name, font_id)				*/
...
void	install_fontlist_for_cycle    (void);
...
/*			PANEL_CYCLE					*/
/*	name	Hier liegt der Zeichensatz als Datei.		*/
...
/*	int	insert_font (name, font_id, insert_position)	*/
/*									*/
/*	Fuegt einen Zeichensatz aus den File name mit font_id	*/
...
/*	  die Liste an							*/
/*									*/
/*	Kann die Datei unter name nicht gefunden werden und		*/
/*	ist name kein absoluter Pfad, so wird in den im		*/
...
/*	int	add_font (name, font_id)				*/
/*									*/
/*	= insert_font (name, font_id, fonts.number_of_fonts), d.h.	*/
...
int	insert_font (char *name, char *font_id, int insert_position)
{
	Graphed_font	new_font;
	int		i;


	if ((i = find_font (name, font_id)) != -1)
		return i;
...
/*
	if ( (new_font->xvFont = (Xv_Font)xv_find(base_frame,FONT,FONT_NAME,name,NULL) ) == (Xv_Font)NULL ) {
		error ("Can't find font %s\n", name);
		return (-1);
	}
*/
	new_font->used = 0;
	strcpy (new_font->id       = mymalloc (strlen(font_id)+1),  font_id);
	strcpy (new_font->name = mymalloc (strlen(name)+1), name);

	/* Verschiebe fonts.list und fonts.list_for_cycle, um Platz zu	*/
	/* machen							*/
	/* Pity poor dog who has to change the next lines ...		*/
	/** The pity poor dog's name is Frank Boerncke, Passau **/
...
/**	fonts.list_for_cycle
		...
		(char *)new_font->font;**/ /** I am a pity poor dog **/

	/** Was hier genau passiert, weiss ich nicht, aber auf diese **/
	/** Weise scheint es zu laufen ... **/
...
int	add_font (char *name, char *font_id)
{
	return	insert_font (name, font_id, fonts.number_of_fonts);
}



int	delete_font (int delete_position)
{
...
Graphed_font	get_current_nodefont (void)
{
	return	fonts.list [current_nodefont_index];
}

Graphed_font	get_current_edgefont (void)
{
...
/*----------------------------------------------------------------------*/
/*									*/
/*	int	find_font (name, font_id)				*/
/*									*/
/*	Sucht Font mit dem name und font_id.			*/
...
Graphed_font	use_font (int font_index)
{
	Xv_font new_font;

	if (fonts.list[font_index]->used == 0) {
		new_font = (Xv_Font)xv_find (base_frame, FONT,
			FONT_NAME, fonts.list[font_index]->name,
			NULL);
		if (new_font == (Xv_Font)NULL) {
			error ("Can't find font %s\n", fonts.list[font_index]->name);
			new_font = (Xv_Font)xv_find (base_frame, FONT, NULL);
		} else {
			fonts.list[font_index]->xvFont = new_font;
		}
	}
...
void		unuse_font (Graphed_font font)
{
...
int		get_font_index (Graphed_font font)
{
	int	i;
...
int	find_font (char *name, char *font_id)
{
	int	i;
	
	for (i=0; i < fonts.number_of_fonts; i++)
	    if (!strcmp (fonts.list[i]->name, name) &&
...
char	**get_fontlist_for_cycle (void)
{	
	return fonts.list_for_cycle;
}
...
void	install_current_nodefont (void)
{	
...
void	install_current_edgefont (void)
{	
...
void	write_fonts (FILE *file)
{
	int	i;
	
	for (i=0; i < fonts.number_of_fonts; i++) {
	  	fprintf (file, "Font:\t%d\t", i);
		write_quoted_text (file, fonts.list[i]->name);
		fprintf (file, "\t");
