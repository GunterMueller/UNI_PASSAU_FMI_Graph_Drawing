...
static	void delete_current_type(void);
static	void close_type_subframe(void);
static 	void	local_insert_type(char *dir, char *file);
static	void make_type_panel_items(Panel panel);
...
void		init_type_fileselector(void)
{	
	if( first ) {
		first = FALSE;
...
void		show_type_edit_subframe (Node_or_edge node_or_edge)
{	
	lock_user_interface ();
...
void	write_type_fileselector(FILE *file)
{
	init_type_fileselector ();
	if (type_fileselector != NULL) {
