...
extern char *get_possible_fileselector_startup_filename (void);
extern void reset_buffer_has_changed (int buffer);
extern void init_print_fileselector (void);
extern void init_type_fileselector(void);
extern void init_font_fileselector (void);
extern void init_file_fileselector (void);
extern void write_print_fileselector(FILE *file);
extern void write_load_store_fileselector(FILE *file);
extern void write_type_fileselector(FILE *file);
extern void write_font_fileselector(FILE *file);
...
static	void	write_graphs             (FILE *file, int buffer);
static	void	write_graph              (FILE *file, Graph graph);
static	void	write_plain_graph        (FILE *file, Graph graph);
static	void	write_graph_internals    (FILE *file);
static	void	write_edge_sourcelist    (FILE *file, register Edge edge);
static	void	write_node               (FILE *file, Node node);
static	void	write_edge               (FILE *file, Edge edge);
static	void	write_node_internals     (FILE *file, Node node);
static	void	write_node_attributes    (FILE *file, Node_attributes attr);
static	void	write_edge_internals     (FILE *file, Edge edge);
static	void	write_edge_attributes    (FILE *file, Edge_attributes attr);
static	void	write_node_style         (FILE *file, char *name, int n);
static	void	write_edge_style         (FILE *file, char *name, int n);
static	void	write_edgeline_internals (FILE *file, Edgeline el_head);
static	void	write_graph_state_list   (FILE *file);
static	void	write_graphed_state_list (FILE *file);
...
int	store_graph (Graph graph, char *filename)
{
	Store_info	info;
...
int	store_graphs (char *filename)
{
	Store_info	info;
...
int		store (Store_info info, char *filename)
{
	FILE	*file;
	...
	buffer_set_filename (wac_buffer, filename);
	...
	if (get_graphed_snooping() == GRAPHED_SNOOPING) {

		FILE	*snoop_file;
		char	snoop_file_out [FILENAMESIZE];

		sprintf (snoop_file_out, "%s.out", filename);
		snoop_file = fopen (snoop_file_out, "w");
		if (snoop_file != (FILE *)NULL) {
			fclose (snoop_file);
		}
	}
	return TRUE;
