...
Edge		create_edge (Node snode, Node tnode)
{
	return create_edge_internal (snode, tnode, -1, NULL);
}

Edge	create_edge_with_sedge (Node snode, Node tnode, char *sedge)
{
	return create_edge_internal (snode, tnode, -1, sedge);
}

Edge	create_edge_internal (Node snode, Node tnode, int nr, char *sedge)
{
	...
	new_edge->label.after_el = empty_edgeline;
	...
	if (sedge == NULL) {
		attatch_sedge (new_edge);
	} else {
		new_edge->sgraph_edge = sedge;
	}
...
void	delete_edge (Edge edge)
{
	...
	free_file_attributes (edge->file_attrs);
...
Edge	copy_edge_without_line (Graph graph, Edge edge)
{
	register	Edge	new_edge;
...
#ifdef LP_LAYOUT
	copy_lp_edge_without_line( new_edge, edge );
#endif
...
Edge	copy_edge (Graph graph, Edge edge)
{
	Edge	new_edge;
...
void	set_edge_file_attributes (Edge edge, File_attributes attrs)
{
	edge->file_attrs = attrs;
}


File_attributes	get_edge_file_attributes (Edge edge)
{
	return edge->file_attrs;
}
...
void	edge_set (Edge edge, Set_attribute attr, ...)
{
	...
	if (set_label) {
		...
		if (text == NULL) {
			...
			if (edge->label.text_to_draw != NULL) {
				myfree (edge->label.text_to_draw);
				edge->label.text_to_draw = (char **)NULL;
			}
...
Edge_attributes	get_edge_attributes (Edge edge)
{
...
void		move_edge (Edge edge, Edgeline el, int x, int y)
{
...
void		extend_edge (Edge edge, Edgeline el, int x, int y)
{
	/* el != edge->line->pre */
...
void		comprime_edge (Edge edge, Edgeline el)
{
	if (is_single_edgeline (edge->line)) {
...
void	set_edge_marked (Edge edge, Marked marked)
{
	switch (marked) {
	     case NOT_MARKED :
		if (is_marked(edge))
...
void	set_edge_to_be_marked (Edge edge)
{
	switch (edge->marked) {
	    case NOT_MARKED :
		break;
...
void	create_edge_attributes (Edge edge, char	*(*create_edge_attr_proc)())
{
	edge->attr = create_edge_attr_proc (edge);
}


void	push_edge_attributes (Edge edge)
{
...
void	pop_edge_attributes (Edge edge, void (*destroy_edge_attr_proc)())
{
