...
/*	edit_mode_event_proc. They process events in their own way,	*/
...
Evp_result	drag_node_proc (Xv_Window window, Event *event, Drag_node_info *info)
{
	...
	if (info->what == MOVE_NODE) {
		...
		if (node != empty_node) {
			...
			if (get_graphed_constrained() || shift_is_down)
				constrain_event (event, node_x(node), node_y(node));
		} else /* node == empty_node */ if ((get_graphed_constrained() || shift_is_down) && get_gridwidth (wac_buffer) > 0)
		...
		if (shift_is_down || get_graphed_constrained()) {
	...
	switch (event_id (event)) {
		...
		if (event_is_down(event) && !dragging) {
			...
			call_user_event_func (UEV_DRAG_NODE, UEV_START, event, (char *)info);
		...
		if (dragging) {
			...
			call_user_event_func (UEV_DRAG_NODE, UEV_DRAG, event, (char *)info);
...
*/
	...
	switch (result) {
	    case EVP_FINISHED :
		call_user_event_func (UEV_DRAG_NODE, UEV_FINISH, event, (char *)info);
		break;
	    case EVP_ERROR :
		call_user_event_func (UEV_DRAG_NODE, UEV_ERROR, event, (char *)info);
		break;
	    default: break;
	}
...
Evp_result	drag_edge_proc (Xv_Window window, Event *event, Drag_edge_info *info)
{
	...
	/* Vorverarbeitung	*/
	if (get_graphed_constrained() || shift_is_down) switch (info->what) {
	    case NEW_EDGE :
	...
	switch (event_id (event)) {
		...
		if (event_is_down(event) && !dragging) {
			...
			call_user_event_func (UEV_DRAG_EDGE, UEV_START, event, (char *)info);
		...
		if (dragging) {
			...
			call_user_event_func (UEV_DRAG_EDGE, UEV_DRAG, event, (char *)info);
...
*/
	...
	switch (result) {
	    case EVP_FINISHED :
		call_user_event_func (UEV_DRAG_EDGE, UEV_FINISH, event, (char *)info);
		break;
	    case EVP_ERROR :
		call_user_event_func (UEV_DRAG_EDGE, UEV_ERROR, event, (char *)info);
		break;
	    default: break;
	}
...
Evp_result		drag_group_box_proc (Xv_Window window, Event *event, Drag_group_box_info *info)
{
...
Evp_result	drag_group_proc (Xv_Window window, Event *event, Drag_group_info *info)
{
	...
	switch (event_id (event)) {
		...
		if (event_is_down(event) && !dragging) {
			...
			call_user_event_func (UEV_DRAG_GROUP, UEV_START, event, (char *)info);
			if (info->do_default_action) {
		...
		if (dragging) {		
			...
			call_user_event_func (UEV_DRAG_GROUP, UEV_DRAG, event, (char *)info);
			x = info->x;
			y = info->y;
...
*/
	...
	switch (result) {
	    case EVP_FINISHED :
		call_user_event_func (UEV_DRAG_GROUP, UEV_FINISH, event, (char *)info);
		break;
	    case EVP_ERROR :
		call_user_event_func (UEV_DRAG_GROUP, UEV_ERROR, event, (char *)info);
		break;
	    default: break;
	}
