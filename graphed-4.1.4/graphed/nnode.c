...
extern void   get_buffer_rect (int buffer, Rect *rect);

#define UNDEFINED_COORDINATES -424242
...
Node	create_node_internal (Graph graph, int nr, char *snode);
...
Node	create_node (Graph graph)
{
	return create_node_internal (graph, -1, NULL);
}



Node	create_node_with_snode (Graph graph, char *snode)
{
	return create_node_internal (graph, -1, snode);
}
...
Node	create_node_internal (Graph graph, int nr, char *snode)
{
	...
	new_node->x                   = UNDEFINED_COORDINATES;
	new_node->y                   = UNDEFINED_COORDINATES;
	...
	new_node->derivation_history  = NULL;
	new_node->sgraph_node         = NULL;
	new_node->file_attrs          = NULL;

#ifdef LP_LAYOUT
	init_lp_node( new_node );
#endif
	...
	if (snode == NULL) {
		attatch_snode (new_node);
	} else {
		new_node->sgraph_node = snode;
	}
...
/*
	if (node->derivation_history != NULL) {
		remove_node (node->derivation_history);
	}
*/
	...
	free_file_attributes (node->file_attrs);
...
Node	copy_node (Graph graph, Node node)
{
	register	Node	new_node;
...
void		set_node_file_attributes (Node node, File_attributes attrs)
{
	node->file_attrs = attrs;
}


File_attributes	get_node_file_attributes (Node node)
{
	return node->file_attrs;
}
...
void	node_set (Node node, Set_attribute attr, ...)
{
	va_list		args;
...
#ifdef LP_LAYOUT
	if( node->graph->lp_graph.derivation_net ) {
	...
	while (attr != SET_ATTRIBUTE_END) {
		switch (attr) {
			...
			x = va_arg (args, int);
			y = va_arg (args, int);
			get_buffer_rect (node->graph->buffer, &buffer_rect);
			if (x < rect_left(&buffer_rect)) {
				x = rect_left(&buffer_rect);
			} else if (x > rect_right(&buffer_rect)) {
				x = rect_right(&buffer_rect);
			}
			if (y < rect_top (&buffer_rect)) {
				y = rect_top (&buffer_rect);
			} else if (y > rect_bottom (&buffer_rect)) {
				y = rect_bottom (&buffer_rect);
			}
			redraw_node  = TRUE;
			redraw_edges = TRUE;
			adjust_nlp   = TRUE;
	...
	/* Unmark && erase	*/
	if (!restore_node && !graphed_state.loading) {
...
Node_attributes	get_node_attributes (Node node)
{
...
void	set_node_marked (Node node, Marked marked)
{
	switch (marked) {
	     case NOT_MARKED :
		if (is_marked(node))
...
void	set_node_to_be_marked (Node node)
{
	switch (node->marked) {
	     case NOT_MARKED :
		break;
...
Node	find_node_with_number (Graph graph, int number)
{
...
Node	get_node_with_number (Graph graph, int number)
{
	Node	node;
	...
	if ( (node = find_node_with_number (graph, number)) != empty_node) {
		...
		return create_node_internal (graph, number, NULL);
	}
}
/************************************************************************/
/*									*/
...
/************************************************************************/
/*									*/
/*	returns -1 when successful, the number of the missing node	*/
/*	otherwise							*/
/*									*/
/************************************************************************/
...
int	all_nodes_complete ()
{
	...
	for_all_graphs (wac_buffer, graph)
	    for_nodes (graph, node)
		if (node_x(node) == UNDEFINED_COORDINATES) return node->nr;
	    end_for_nodes (graph, node);
...
void	create_node_attributes (Node node, char	*(*create_node_attr_proc)())
{
	node->attr = create_node_attr_proc (node);
}


void	push_node_attributes (Node node)
{
...
void	pop_node_attributes    (Node node, void	(*destroy_node_attr_proc)())
{
