...
/*	wird an eine ueber ui_state.user_event_proc angesprochene	*/
/*	Prozedur uebergeben (dazu siehe die folgenden Abschnitte).	*/
/*									*/
...
/*	innerhalb einer Scrollbar befindet, an die			*/
/*	ui_state.user_event_proc uebergeben.				*/
/*									*/
/************************************************************************/
...
void		change_working_area(Xv_window window)
{
...
void	working_area_event_proc (Xv_Window xv_window, Event *event, Notify_arg arg)
         	           /* MH */
          	    ...
          	    
{
	...
	for (i=0; i<N_BUFFERS; i++) if (buffers[i].used) {
		if ((canvases[i].pixwin) == xv_window) {
	...
	/* A little hack to scroll the working area when a graph is	*/
	/* loaded on startup.						*/
	if (!graphed_state.loading &&
	    !graphed_state.startup &&
	    this_buffer <= N_BUFFERS &&
	    !canvases[this_buffer].canvas_seen_by_working_area_event_proc) {

		canvases[this_buffer].canvas_seen_by_working_area_event_proc = TRUE;

		if (canvases[this_buffer].startup_scroll_x == -42 ||
		    canvases[this_buffer].startup_scroll_y == -42) {
			if (buffers[this_buffer].graphs == empty_graph) {
				canvases[this_buffer].startup_scroll_x = 
					  (int)xv_get(canvases[this_buffer].canvas, CANVAS_WIDTH)/2
					- (int)xv_get(canvases[this_buffer].canvas, XV_WIDTH) / 2;
				canvases[this_buffer].startup_scroll_y =
					  (int)xv_get(canvases[this_buffer].canvas, CANVAS_HEIGHT)/2
					- (int)xv_get(canvases[this_buffer].canvas, XV_HEIGHT) / 2;
			} else {
				Rect	rect;
				rect = compute_rect_around_graphs (this_buffer);
				canvases[this_buffer].startup_scroll_x = rect_left(&rect);
				canvases[this_buffer].startup_scroll_y = rect_top (&rect);
			}
		}

		scroll_buffer (this_buffer,
			canvases[this_buffer].startup_scroll_x,
			canvases[this_buffer].startup_scroll_y);
	}
	...
	if (!illegal_event &&
	    !inside_scrollbar &&
	    (ui_state.dragging_node      ||
	     ui_state.dragging_edge      ||
	     ui_state.dragging_group     ||
	     ui_state.dragging_group_box ||
	     ui_state.making_node        ||
	     ui_state.making_edge)) {
	
		(void)ui_state.user_event_proc (window, event, EVP_CONSUME);

	} else if ((!illegal_event &&
	           event_is_ascii(event) && ctrl_is_down && event_id(event) != CTRL_M) ||
	           (event_is_meta (event))) switch (event_id (event)) {
		...
		if (event_is_down(event) &&
		    !ui_state.dragging_node  && !ui_state.dragging_edge &&
		    !ui_state.dragging_group && !ui_state.dragging_group_box &&
		    !ui_state.making_node && !ui_state.making_edge) {
			menu_show (main_menu,
				xv_window,
				event,
				0);
	    ...
	    case KEY_TOP (5) :
		if (shift_is_down) {
			dispatch_user_action (SET_GRID_8_8);
		} else {
			/* dispatch_user_action (GRAPHED_DEFAULTS); soon to come ... */
		}
		break;
		...
		if (shift_is_down) {
			...
			dispatch_user_action (NODE_DEFAULTS);
		}
		break;
		...
		if (shift_is_down) {
			...
			dispatch_user_action (EDGE_DEFAULTS);
		}
		break;
		...
		if (shift_is_down) {
			...
			dispatch_user_action (EDIT_GRAGRA);
		}
		break;
		...
		else if (ui_state.user_event_proc == edit_mode_event_proc)
	    ...
	    case LOC_MOVE       :    /* Dieser Event wird nicht an	*/
	    			     /* ui_state.user_event_proc weitergegeben	*/
		break;
...
/*	aktiv, naehmlich die, die mit ui_state.user_event_proc		*/
/*	angesprochen wird.						*/
...
/*									*/
/*	Entfernt ui_state.user_event_proc, falls sie das erlaubt	*/
/*	(EVP_OK).							*/
...
/*									*/
/*	Setzt proc als ui_state.user_event_proc ein. Falls Das Einsetzen*/
/*	erfolgreich war, Rueckgabe TRUE, sonst FALSE (und Einsetzen	*/
/*	von default_user_event_proc als Ersatz).			*/
/*									*/
/*	WICHTIG : Die alte ui_state.user_event_proc muss vorher mit der	*/
...
int	remove_user_event_proc (void)
{
	if (ui_state.user_event_proc ((Xv_Window)NULL,(Event *)NULL,EVP_SHUTDOWN) == EVP_OK) {
		ui_state.user_event_proc = default_user_event_proc;
		return TRUE;
	} else
		return FALSE;
}
...
int	set_user_event_proc (Evp_result (*proc) ())
{
	if (proc (working_area_canvas, (Event *)NULL, EVP_STARTUP) == EVP_OK) {
		ui_state.user_event_proc = proc;
		return TRUE;
	} else
		return FALSE;
}
...
Evp_result	default_user_event_proc (Xv_Window window, Event *event, Message_to_event_proc message)
{
...
Picklist	dispatch_picklist (Picklist pl, int complain_if_empty)
{	
		...
		if (ctrl_is_down) {
			...
			while (pl != empty_picklist) {
				...
				switch (notice_prompt (base_frame, NULL,	/*fisprompt*/
					...
					NULL)) {
				    case PROMPT_ACCEPT :
					ms_left_down = ms_right_down = ms_middle_down = FALSE;
					return pl;
					break;
				    case PROMPT_REFUSE :
					ms_left_down = ms_right_down = ms_middle_down = FALSE;
...
int	graph_is_picked (Graph graph)
{
	return something_picked &&
...
Picklist	get_picked_object (void)
{
	if (something_picked)
...
Group	get_picked_group  (void)
{
	if (something_picked && picked_object->what == GROUP_PICKED)
		return picked_object->which.group;
	else
		return empty_group;
}


Graph	get_picked_graph  (void)
{
...
Graph	get_picked_or_only_existent_graph (void)
{
	Graph	graph;
...
Edge	get_last_edge (void)
{
	return last.edge;
}


Graph	get_last_graph (void)
{
	return last.graph;
}
...
void	set_last_node (Node node)
{
	last.node = node;
}


void	set_last_edge (Edge edge)
{
	last.edge = edge;
}


void	set_last_graph (Graph graph)
{
	last.graph = graph;
}
...
void	constrain_event (Event *event, int x, int y)
{
	int	evx, evy;
...
void	constrain_length_event (Event *event, int x, int y)
{
...
{
	ui_state.user_event_proc = default_user_event_proc;
	ui_state.making_node = FALSE;
	ui_state.making_edge = FALSE;
...
void	lock_user_interface (void)
{
	user_interface_locked = TRUE;
}


void	unlock_user_interface (void)
{
	user_interface_locked = FALSE;
}


int	test_user_interface_locked (void)
{
...
int	user_interface_check_destroy_buffer (int buffer)
{
	Graph	graph;
	Node	node;
	Edge	edge;
...
Rect		compute_rect_around_selection (Picklist selection)
{
	Rect	r;
	...
	rect_construct (&r, 0,0,0,0); /* else ;-) */
	return r;
}


Rect	compute_rect_around_current_selection (void)
{
	return compute_rect_around_selection (get_picked_object ());
}


Rect	compute_rect_around_graph_of_current_selection (void)
{
	Graph	g;
	Rect	r;
...
void	compute_subwindow_position_at_graph_of_current_selection (Xv_Window window)
{
	int	x,y;
	Rect	rect;
	Graph	g;
...
void	compute_subwindow_position_at_current_selection (Xv_Window window)
{
	int	x,y;
	Rect	rect;
