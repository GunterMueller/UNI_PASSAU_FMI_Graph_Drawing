...
static void 	nf_set_directory(Panel_item item, Event *event);	/* called if CR was pressed while editing directory	*/
static void 	nf_react_to_selection(Panel_item item, char *string, caddr_t client_data, Panel_list_op op, Event *event);/* called if user chose directory or file with mouse	*/
static void 	nf_set_current_mask(Panel_item item, unsigned int value, Event *event);	/* called if user chose an extension with mouse		*/
static void 	nf_quit_proc(Panel_item item, Event *event);		/* called if user chose 'done' in frame menu		*/
static void 	nf_set_display_status(Panel_item item, unsigned int value, Event *event);/* called if user changed display-choice		*/

static Fs_item	fls_get_fsi(Panel_item item);		/* returns fileselector-descriptor of 'item'		*/

static int	fls_is_directory(char *name, int *eno);	/* returns TRUE if file 'name' is a directory; accesses drive	*/
static int	fits_reg_expr(char *str, char *exp);	/* returns TRUE if 'str' fits the "regular expression" 'exp' 	*/
static void 	fls_set_filename(Panel_item item);	/* called while editing file-input-entry		*/

static void	fls_create_window(Fs_item fsi);	/* creates whole fileselector window			*/
static void	fls_finish(Fs_item fsi, char *dir, char *file, int close);		/* always called when quitting a fileselector		*/
static void	fls_browse_proc(Fs_item fsi);	/* browses directory to show_panel; uses selection service */

static	void	scan_open_file(char *name);	/* opens file 'name' for reading fileselector-contents	*/
static	void	scan_close_file(void);	/* closes an open setup-file				*/
static	int	scan_get_next_line(void);	/* reads next line from scanfile; returns TRUE upon success	*/
static	int	scan_find_string(char *str);	/* tries to find 'str' in scanfile beginning at scan_position	*/
static	int	is_number(char *s);		/* returns TRUE if 's' represents an integer value	*/
...
void	fls_set_extension(Fs_item fsi, int no, char *value); 		/* forward declaration */

Fs_item	fls_create(void)			/* creates a fileselector and sets its default values */
...
void	fls_close(Fs_item fsi)			/* removes fileselector from screen and releases display-list-items */
...
void	fls_destroy(Fs_item *fsi)		/* destroys a fileselector and releases all its allocated memory */
...
void	fls_set_user_panel_items_create_proc(Fs_item fsi, void (*user_proc) (/* ??? */))	/* user's chance to add an own panel to */
       	    							/* the fileselector */
...
void	fls_set_extension(Fs_item fsi, int no, char *value)	/* sets the extension associated with 'no' to 'value' */
...
void	fls_set_current_extension(Fs_item fsi, int no)	/* the value of the extension 'no' becomes the current mask */
...
void	fls_copy_attributes(Fs_item dest, Fs_item source)	/* copies the user-relevant data of a fileselector */
       	             				/* to an other */
{
	int	i;
...
void	fls_set_working_directory(Fs_item fsi, char *dir)	/* sets the default value for the directory item */
       	    
    	     
{
	if(fsi != (Fs_item)NULL){
		if ((int)strlen(dir) < WDLEN) {
			strcpy(fsi->cwd, dir);
		} else {
...
void	fls_set_default_filename(Fs_item fsi, char *file)	/* sets the default value for the file_item */
       	    
    	      
{
	if(fsi != (Fs_item)NULL){
		if ((int)strlen(file) < FNLEN) {
...
void	fls_set_info(Fs_item fsi, char *info)		/* sets the fileselector's label */
       	    
    	      
{
	if(fsi != (Fs_item)NULL){
		if ((int)strlen(info) < SILEN) {
...
int	fls_busy(Fs_item fsi)			/* returns TRUE if the fileselector is on screen */
...
void	fls_set_auto_destroy(Fs_item fsi, int bool_value)	/* set the auto_destroy-status of a fileselector */
...
void	fileselect(Fs_item fsi, Frame parent_frame, void (*done_proc) (/* ??? */))	/* pushes a fileselector on screen and calls */
       	    						/* done_proc when the user has made up his/her mind */
...
static void nf_set_directory(Panel_item item, Event *event)	/* called if CR has been pressed while editing directory */
...
static void nf_file_input_event_proc(Panel_item item, Event *event)	/* notify-proc. for file_input_item	*/
...
static void nf_react_to_selection(Panel_item item, char *string, caddr_t client_data, Panel_list_op op, Event *event)
{
	int		i;
	Fs_item		fsi;
...
static void nf_set_current_mask(Panel_item item, unsigned int value, Event *event)	/* called if user has chosen an extension with mouse */
...
static void nf_edit_extension(Panel_item item, Event *event)	/* called if the user edited an extension and pressed RETURN */
...
static void nf_frame_done_proc(Frame frame)
{
	Fs_item	fsi;
...
static void nf_quit_proc(Panel_item item, Event *event)
{
	Fs_item	fsi;
...
static void nf_refresh_proc(Panel_item item, Event *event)	/* called if user has pressed refresh-button */
          	     				/* forces the fileselector to rescan the directory */
...
static void nf_addextension_proc(Panel_item item, Event *event)	/* called if user has pressed 'fit extension'-button */
          	     				/* switches the value of fit_extension (boolean) */
...
static void nf_point_files(Panel_item item, Event *event)		/* called if user has pressed '.'-files -button */
          	     				/* switches the value of show_point_files (boolean) */
...
static void nf_set_display_status(Panel_item item, unsigned int value, Event *event)	/* called if user has changed display-choice */
...
static int fls_is_directory(char *name, int *eno)		/* returns TRUE if file 'name' is a directory; accesses drive */
    	      					/* if something got wrong: returns FALSE and sets eno to 1 */
...
static int fits_reg_expr(char *str, char *exp)	/* evaluation of very small regular expressions with only */
    	           			/* '*' and '?' as wildcards */
{
       	    ...
       	    
{
	...
	if(fsi->parent_frame != (Frame)NULL){
		...
		screenrect   = (Rect *)xv_get(fsi->select_frame, WIN_SCREEN_RECT);
		screenwidth  = screenrect->r_width;
		screenheight = screenrect->r_height;
	
		xv_set(fsi->select_frame,
#if FALSE
			WIN_X,	((screenwidth - xv_get(fsi->select_frame, XV_WIDTH))/2 -
				xv_get(fsi->parent_frame, WIN_X)),
			WIN_Y,	((screenheight - xv_get(fsi->select_frame, XV_HEIGHT))/2 -
...
static void fls_finish(Fs_item fsi, char *dir, char *file, int close)	/* does the final jobs of the fileselector */
...
static int namecmp(struct filent *filent1, struct filent *filent2)
{
...
static void quick_dir_sort(struct filent **file_list, int nr_of_filent)
{
...
static void fls_browse_proc(Fs_item fsi)	/* browses directory to show_panel; uses selection mask */
       	    
{
	int		i, nr_of_filent, eno;
	char		name[MAXNAMLEN+6], help[WDLEN];
#ifndef _BSD_SOURCE
	struct dirent	*nl;
	DIR 		*current_dir;
#else
	...
	if (current_dir != NULL) {
		...
		for(i=1; i<nr_of_filent; i++) {
			nl = readdir(current_dir);
			file_list[i] = (struct filent*)NULL;
			if (nl->d_name[0] != '.' || nl->d_name[1] != '.'|| nl->d_name[2] != '\0')
			{
				if(fsi->show_point_files || nl->d_name[0] != '.') {
					if(fls_is_directory(nl->d_name, &eno)) {
						if(fsi->dir_cycle > 0) {
							file_list[i] = (struct filent*)malloc(sizeof(struct filent));
							strcpy(file_list[i]->filename, nl->d_name);
							file_list[i]->is_file = FALSE;
							fsi->nr_of_filent++;
						}
					} else {
						if(fsi->dir_cycle < 2 && fits_reg_expr(nl->d_name, fsi->extension[fsi->last_extension])) {
							file_list[i] = (struct filent*)malloc(sizeof(struct filent));
							strcpy(file_list[i]->filename, nl->d_name);
		...
		for(i=1; i<nr_of_filent; i++) {
			file_list[i] = (struct filent*)NULL;
			if (nl[i]->d_name[0] != '.' || nl[i]->d_name[1] != '.'|| nl[i]->d_name[2] != '\0')
			{
		...
		fsi->file_list = (struct filent**)malloc(fsi->nr_of_filent*sizeof(struct filent*));
		...
		for(i=0; i<(fsi->nr_of_filent); i++) {
			sprintf(name, "%s%s", fsi->file_list[i]->is_file ? "     " : "<> ", fsi->file_list[i]->filename);
...
static void fls_set_filename(Panel_item item)	/* 'item' is always the file_input_item of a fileselector. 	*/
...
static void scan_open_file(char *name)
{
...
static void scan_close_file(void)
{
	if(scanfile != (FILE *)NULL) {
		fclose(scanfile);
	}
...
static int scan_get_next_line(void)
{
	char dummy;
	if(scanfile == (FILE *)NULL) {
		return FALSE;
...
static int scan_find_string(char *str)
{
	do {
...
void	fls_setup_from_file(Fs_item fsi, char *filename, char *identification)
{
	...
	if(found) {
					...
					if(!strcmp(key, "hidden")) {
						...
						if(!strcmp(key, "displayed")) {
							...
							fprintf(stderr, "fls_setup - error in \"%s\", line %d:\n expecting 'hidden' or 'displayed' instead of '%s'.\n",
								(filename ? filename : ""), linecount, key);
						}
					}
				}
					...
					if(is_number(key)) {
						...
						if(val >= 0 && val < MASKNO) {
							...
							fprintf(stderr, "fls_setup - error in \"%s\", line %d:\n number out of range [1..%d]\n",
								(filename ? filename : ""), linecount, MASKNO);
						}
					} else {
		...
		fprintf(stderr, "fls_setup - error in \"%s\":\n fileselector information \"%s\" not found.\n",
			(filename ? filename : ""), identification);
	}
	scan_close_file();
}


void	fls_write_to_file(Fs_item fsi, FILE *file, char *identification)
{
	int i;
