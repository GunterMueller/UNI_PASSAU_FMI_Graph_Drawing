...
static int	fls_fits_regular_expression(char *str, char *exp)
{
...
static Komp_list	new_komplist(void)
{
	Komp_list hilf;
...
void	dispose_komplist(Komp_list k)
{
	Komp_list h;
...
Komp_list	filename_komponents(char *name)
{
...
Komp_list	rexp_komponents(char *name)
{
	struct komp_list	hilf;
	Komp_list		h,hh;
	register int		len, pos1, pos2;
...
/*
static void print_komplist(Komp_list k, char *s)
{
...
static void	special_add_string(char *s1, char *s2)
{
	int h=0,hh=0;
	char	c;
...
char	*get_name_of_lists(Komp_list nkl, Komp_list ekl)
{
