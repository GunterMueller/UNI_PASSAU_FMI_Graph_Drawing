...
static	void	create_rt_subframe	(void);
static	void	save_rt_settings	(void);
static	void	notify_rt_buttons (Panel_item item, Event *event);
...
static	void	save_rt_settings (void)
{
	saved_rt_minsep              = atoi ((char *)xv_get(rt_text1, PANEL_VALUE));
	saved_rt_correction_of_tree  = atoi ((char *)xv_get(rt_text2, PANEL_VALUE));
	saved_rt_feasible_difference = atoi ((char *)xv_get(rt_text3, PANEL_VALUE));
	saved_rt_lower_equal         = atoi ((char *)xv_get(rt_text4, PANEL_VALUE));
	saved_rt_pixel_per_unit      = atoi ((char *)xv_get(rt_text5, PANEL_VALUE));
}
