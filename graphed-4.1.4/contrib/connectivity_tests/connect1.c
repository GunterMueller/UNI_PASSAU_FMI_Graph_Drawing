...
static void dfs_in_and_out (Snode n)
{
    Sedge e;
    attr_flags(n) = TRUE;
    for_sourcelist(n,e)
...
/*--------------------------- connectivity test -----------------------------*/
bool test_sgraph_connected(Sgraph g)
{
    Snode n;
...
static int init_attrs(Sgraph g)
{
    Snode n;
    Numbers dummy;
    int i = 0;
...
static void clean_attrs(Sgraph g)
{
    Snode n;
    for_all_nodes(g,n)
    {
...
/* not used
static
int snode_stack_isempty(SnodeStack st)
{
...
static bool is_on_snode_stack(SnodeStack st, Snode n)
{
...
static
SnodeStack snode_stack_push(SnodeStack st, Snode n)
{
    SnodeStack dummy;
    if (n != empty_snode)
    {
...
static
Snode snode_stack_top(SnodeStack st)
{
    return st->snode;
}

static
SnodeStack snode_stack_pop(SnodeStack st)
{
    SnodeStack dummy;
    dummy = st;
...
static void searchc(Snode n)		/* Derived from AHO p192 */
...
bool test_sgraph_strongly_connected(Sgraph g)
{
    Snode n;
    bool result;
    if (g == empty_sgraph || g->nodes == empty_node) return TRUE;
...
/*-------------------------- biconnectivity test ----------------------------*/
static bool dfs_bi (Snode n, int *nextnumber)
{
    Sedge e;
    int childs = 0;
...
static int dfs_bi_postorder (Snode n, Snode father)
{
    int lowmin, m;
    Sedge e;
    attr_data_of_type(n, Numbers)->visited = TRUE;
 ...
 */
static bool dfs_bi_check (Snode n)
{
    Sedge e;
...
static bool bi_check(Snode n)                   /* Don`t check root-node */
...
bool test_sgraph_biconnected(Sgraph g)
{
    Snode n;
    int nr = 1;
    bool result;
...
static bool test_sgraph_without_node_strongly_connected(Snode node)
{   
    int maxcount;
    bool result;
    Snode n;
...
bool test_sgraph_strongly_biconnected(Sgraph g)
{
    Snode n;
    if (g == empty_sgraph || g->nodes == empty_node) return TRUE;
    if (!g->directed)
