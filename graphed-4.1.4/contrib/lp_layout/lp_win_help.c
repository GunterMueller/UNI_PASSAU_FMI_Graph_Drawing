...
typedef void (*Cycle_Panel_Notify_Func)(Panel_item item, int value, Event *event);

/********************************************************************************/
/*										*/
/*	MY_CMD_position_absolute						*/
/*										*/
...
void	My_CMD_position_absolute(Frame win, int x, int y)
{
	xv_set( win,	XV_X,	x,
			XV_Y,	y,
			0);
...
void	MY_CMD_position_relative(Frame win, Frame reference, int x, int y)
{
	Rect	ref_rect;
...
void	MY_WIN_position_absolute(Frame win, int x, int y)
{
	Xv_Window	parent;
	Rect		ref_rect;
...
void	MY_WIN_position_relative(Frame win, Frame reference, int x, int y)
{
	Rect	ref_rect;
	
	frame_get_rect( reference, &ref_rect );
	MY_WIN_position_absolute( win, ref_rect.r_left + x, ref_rect.r_top + y );
}

/********************************************************************************/
/*										*/
/*	MY_cycle_panel_choice_stack						*/
/*										*/
...
void	MY_cycle_panel_choice_stack(Panel_item item, int value, Event *event)
{
	Cycle_Panel_Notify_Func user_notify_proc;
	int	number_of_choices;
	
	xv_set( item,	PANEL_VALUE,		value,
			0);

	user_notify_proc = (Cycle_Panel_Notify_Func)xv_get( item, PANEL_CLIENT_DATA );
...
void	My_show_panel_item_proc(Panel_item item, int value, Event *event)
{
