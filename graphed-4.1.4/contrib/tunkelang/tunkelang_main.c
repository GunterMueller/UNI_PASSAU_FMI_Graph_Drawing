...
best_cuts_of_node=cuts_of_node;
 ...
 cut_number=cut_number+best_cuts_of_node;
...
struct nachfolger *pointer;
 ...
 /* else printf("At maximum depth in recursion.\n"); */
}
...
else
{
 ...
 if(count_cuts&&scan_corners&&(best_cuts>=1))
 {
  ...
  if(corner_update)
  {
   ...
   printf("Corner-scan successful. Coord: (%d,%d), barycenter is (%d,%d)\n",best_x,best_y,bary_center_x,bary_center_y);  
  }
 } 
...
char   message_string[50];
 ...
 for(i=1;i<=nodes;i++)
 {
        sprintf(message_string,"STATUS: I/III  %d/%d",i,nodes);
 ...
 for(i=1;i<=nodes;i++)
 {
        sprintf(message_string,"STATUS: II/III  %d/%d",i,nodes);
        display_the_message(message_string);
	...
	if(i>1) 
        ...
        if(count_cuts) printf("Number of cuts is: %d\n",cut_number);

        if(quit_the_algorithm) return;
 }
 if(animation) printf("Fine-tuning begins with image %d\n",number_of_image_to_save);
 ...
 for(j=1;j<=nodes;j++)
 {
  sprintf(message_string,"STATUS: III/III  %d/%d",j,nodes);
  display_the_message(message_string);
 ...
 if(count_cuts) printf("\nNumber of cuts is: %d\n",cut_number);
 
 /* show_new_struct();                  */
 /* save_real_graphcoords("graph.bst"); */

 printf("Fine-tuning-Iterations: %d\n",iterations*16);
}
