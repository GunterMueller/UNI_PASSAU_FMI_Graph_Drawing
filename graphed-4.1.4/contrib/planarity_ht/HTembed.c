...
#define PATH(i) (attr_data_of_type(SNODE[i],NODEATTR)->path)
#define ROOT(i) (attr_data_of_type(SNODE[i],NODEATTR)->root)
...
/* Typdefinitionen fuer zusaetzliche Datenstrukturen, die in
  ...
  struct Edgelist {
                   struct Edgelist *next;      /* Zeiger zum naechsten Listenelement */
                   struct Edgelist *pre;       /* Zeiger fuer doppelt verkettete Liste */
                   struct Edgelist *suc;       /* ('suc' = Nachfolger im Uhrzeigersinn) */
                   struct Edgelist *link;      /* Verbindung zur selben Kante, die in */
...
static Snode *SNODE;     /* Das Feld 'NODE' dient uns dazu ueber die Nummer  */
...
static char *set;     /* Zeichenkette dient uns als Menge von Knoten (BITSET) */
  ...
  /* Hilfsprozedur zum Testen der Zulaessigkeit der Kante (i,j).  */
    ...
    static RESULT test_edge(int i, int j)
{
      if (j == i)
        return SELF_LOOP;
      if (set[j] == SIGN_ON)
...
static RESULT test_graph(Sgraph graph)
{ RESULT result;
  int i;
  Snode node;
  Sedge edge;
...
static void append_edgelist(EDGELIST *list, EDGELIST elem)
{
  elem-> next = *list;
  *list = elem;
}
...
static void init_additional_structures(Sgraph graph)
...
static void link_edges(Sgraph graph)
{ int i,n;
  Snode node;
  EDGELIST elem,dual;
...
static void create_slist_structure(Sgraph graph)
{ Slist slist;
  Snode node;
  EDGELIST elem,last;
...
static void add_to_dependency_graph(int p1, int p2, char kind)
{
...
/* Mit Hilfe der Informationen des 'Dependency' Graphen legen die
  ...
  /* Rekursive Hilfsprozedur zum Festlegen der Seiten; geht die
    ...
    static void fp(int i, char side)
{ HEAP elem;
...
static void fix_paths(int p)
                   /* Nummer des letzten Pfades */
...
static EDGELIST embed_below(EDGELIST elem, EDGELIST suc)
{
...
static EDGELIST embedder(int v, EDGELIST u_v)
{ EDGELIST v_w,save,down,low;
  int w,w0;

  low = (EDGELIST)NULL;
  down = embed_below(u_v-> link,ROOT(v));     /* 'down' initialisieren und evtl. */
  for_all_edges(SNODE[v],v_w)                  /* mit Kanten vorhergehender Kom-  */
    {                                         /* ponenten verbinden.             */
      w = NUMBER(v_w-> node);
      if (v_w-> mark == ARC)
        {
    ...
    static void empty_B(void)
{
  ...
  /* Hinzufuegen eines Elements auf 'B' */
    ...
    static void add_to_B(int x, int y)
{
      top_B++;
      B[top_B].x = x;
      B[top_B].y = y;
  ...
  /* Der Wert des geloeschten Elements wird zurueckgegeben */
    ...
    static void delete_from_B(int *x, int *y)
{
      *x = B[top_B].x;
      *y = B[top_B].y;
      top_B--;
  ...
  /* Ersetzen des obersten Elements durch (x,y) */
    ...
    static void replace_on_B(int x, int y)
{
      B[top_B].x = x;
      B[top_B].y = y;
    }
  ...
  /* Das Resultat ist FALSE, falls kein Eintrag existiert, sonst TRUE */
    ...
    static bool top_on_B(int *x, int *y)
{
...
/* Das Resultat wird NONPLANAR zurueckgegeben, falls der Graph 
  ...
  static RESULT pathfinder(int v)
...
/* Funktion nach der die Kanten in Bucketsort sortiert werden */
    ...
    static int q(Snode v, EDGELIST v_w)
{ Snode w = v_w->node;
...
/* Fuer Details des Algorithmus siehe QUELLE S.556 */
  ...
  static void bucketsort(int v, int w, int t)
{ int i;
    EDGELIST elem;
    ...
    for(i = 2*v; i<= 2*t+1; i++)
      bucket[i] = (EDGELIST)NULL;
    for(i = w; i <= t; i++)
      while((elem = EDGES(SNODE[i])) != (EDGELIST)NULL)
        {
          EDGES(SNODE[i]) = elem-> next;
          append_edgelist(&bucket[q(SNODE[i],elem)],elem);
        }
    for(i = 2*t+1; i >= 2*v; i--)
...
static RESULT biconnected_component(int v, EDGELIST v_w, int w, int t)
{ EDGELIST low;
...
/* Ergaenzend zu obigem Algorithmus wird neben der Bestimmung der
  ...
  static RESULT dfs(Snode v, Snode u)
{ Snode w;
    EDGELIST v_w;    /* Kante von v nach w */
      
    NUMBER(v) = LOWPT1(v) = LOWPT2(v) = ++N;
    SNODE[N] = v;                         /* Zeiger auf 'Snode'-Struktur setzen */
    for_all_edges(v,v_w)
      {
        w = v_w->node;
        if (NUMBER(w) == 0)
...
static RESULT depth_first_search(Sgraph graph)
{ Snode node;
...
{ RESULT result;
  ...
  if ((SNODE = (Snode *)malloc((unsigned)(V+1)*sizeof(Snode))) == (Snode *)NULL)
