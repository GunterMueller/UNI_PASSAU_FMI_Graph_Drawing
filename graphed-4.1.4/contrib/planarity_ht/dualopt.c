...
void message_dualnode(Slist list)
{
   Slist l;
   for_slist(list,l)
     {
...
void message_graph(Sgraph DualGraph)
{
#ifdef DEBUG
   Snode node;
   Sedge edge;
...
static Slist Edge_in_slist(Snode node1, Snode node2)    /* Pointer auf Slist von node2, die auf */
                                           /* Kante(node1,node2) zeigt.            */
...
static Sedge DEdge(Snode node1, Snode node2)
{
...
static void set_plane(Snode node1, Snode node2, Sedge edge12, Plane plane, Side lr)
{
...
static void set_plane_and_append(Snode node1, Snode node2, Sedge edge12, Plane plane, Side lr)
{
...
static Plane get_plane(Snode node1, Snode node2, Side lr)
{
   if (node1 < node2) /* node1 = sourcenode */
     {
      if (lr == left)
        return LEFT(DEdge(node1,node2));
      else
        return RIGHT(DEdge(node1,node2));
     }
   else /* node2 = sourcenode */
     {
      if (lr == left)
...
static Plane make_plane(Snode node1, Snode node2)
{
   Plane plane = (Plane)malloc(sizeof(struct plane));
   plane->planeedges = new_slist(make_attr(ATTR_DATA,(char *)DEdge(node1,node2)));
   plane->visited = 0;
   return plane;
  }
...
static void add_planes(Snode node1, Snode node2, Side lr1, Snode node3, Snode node4, Side lr2)
...
static void dualgr (Snode node1, Snode node2)
{
   Snode leftnode, lastnode, rightnode;
   Slist leftlist, rightlist;
   Sedge leftedge, rightedge;
   PASSED(DEdge(node1,node2)) = 1;      /* Kantenmarkierung */
   if (VISITED(node2) == 0)
     {
      VISITED(node2) = 1;
...
static Snode nextNode (Snode node)
{
   Snode next = NULL;
   Slist l;
...
static void startdual (Snode node)
{
...
static int exist_edge(Snode node1, Snode node2)
{
   int flag = 0;
   Sedge edge;
...
static void make_nodes_and_edges_of_dualgraph(Sgraph DualGraph, Plane plane)
{
   Plane nextplane;
...
static void start_make_nodes_and_edges_of_dualgraph(Sgraph DualGraph, Plane firstplane)
{
...
static Sgraph make_dualgraph(Sgraph DualGraph, Sgraph oldgraph)
{
   Snode node;
   Sedge edge;
   Slist l;
...
static void delete_planes (Sgraph graph)
{
   Snode node;
   Sedge edge;
