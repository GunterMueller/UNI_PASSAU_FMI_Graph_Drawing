...
static Snode *SNODE;     /* Das Feld 'SNODE' dient uns dazu ueber die Nummer  */
                        /* eines Knotens auf das Element der Datenstruktur  */
                        /* zugreifen zu koennen.                            */
                        /* Es gilt:        SNODE[NUMBER(v)] = v              */
                        /*            und  NUMBER(SNODE[i]) = i.             */
                        /* In 'planarity' wird 'SNODE' genuegend Speicher-   */
                        /* platz zugewiesen.                                */
...
static RESULT biconnected_component(int v, int w, int t)
{ 
  bucketsort(v,w,t);
...
{ RESULT result;
  ...
  if ((SNODE = (Snode *)malloc((V+1)*sizeof(Snode))) == (Snode *)NULL)
