...
/*minimal_bends_settings2 minimal_bends_settings2;*/

void	save_minimal_bends_settings2 (void)
{
	if (showing_minimal_bends_subframe()) {
		minimal_bends_settings2.grid = gridder_get_size (gridder);
		minimal_bends_settings2.grid_defaults = (int)gridder_get_value (gridder);
	} else {
		if (minimal_bends_settings2.grid != GRIDDER_DISTANCE_OTHER) {
			minimal_bends_settings2.grid = recompute_gridder_size (
				NULL,
				minimal_bends_settings2.grid_defaults,
				GRIDDER_HEIGHT);
		}
	}
}
...
{
	struct minimal_bends_settings2 settings;
...
void	 	 check_and_call_minimal_bends_layout2 (Sgraph_proc_info info)
{
	...
	if (test_minimal_bends_layout2(info)) {
		minimal_bends_proc (info);
		info->recenter = TRUE;
	} else {
...
{
	save_minimal_bends_settings2 ();
	call_sgraph_proc (check_and_call_minimal_bends_layout2, NULL);
}
...
void menu_minimal_bends_layout2 (Menu menu, Menu_item menu_item)
{
	save_minimal_bends_settings2 ();
	call_sgraph_proc (check_and_call_minimal_bends_layout2, NULL);
}
...
void menu_minimal_bends_layout2_settings (Menu menu, Menu_item menu_item)
{
	save_minimal_bends_settings2 ();
	show_minimal_bends_subframe ();
}
