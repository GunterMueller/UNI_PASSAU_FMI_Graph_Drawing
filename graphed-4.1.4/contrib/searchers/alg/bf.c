...
void    BruteForceSearch(Sgraph sgraph, Method method)
{
	Snode   node, notsetnode;
	Sedge   edge;
	int again = 0;

	SetOn( last_node_in_graph( sgraph ) );
...
if ( method != METHOD_EDGE_SEARCH )
{
	do 
	{
		...
		{
			for_all_adjacent_not_set_nodes( node, notsetnode )
			{
				if ( method == METHOD_NODE_SEARCH )
					SetOnCheck( notsetnode );    
				else
					MoveToCheck( node, notsetnode );
				again = 1;
			} end_for_all_adjacent_not_set_nodes( node, notsetnode );
