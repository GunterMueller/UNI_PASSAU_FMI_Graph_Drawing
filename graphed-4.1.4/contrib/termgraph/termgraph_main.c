...
**************************************************************************
**									**
**	void 	call_termgraph_window_proc(), petrinet_turn_proc(),	**
**		call_petrinet_turn_proc(),				**
**		call_term_to_graph_creation_proc(), 			**
...
extern 	char	*fill_buffer(Sgraph pgraph, char *input_name, char **output_string);
extern 	char	*flatten(char *ch_in);
extern	pnode	*termgraph_parsetree(char *pt, int down, int up);
extern	pnode	*createpnode(void);
extern	int 	show_tree(int k, pnode *father);
extern	int	sequential_order(pnode *p);
extern	int	search_for_successor(pnode *p, pnode *succ_element);
extern	char 	*call_name(int argc, char **argv);
extern	int 	termgraph_draw_petrinet(Sgraph pgraph, FILE *filep, pnode *p, int *num, int argc, char **argv, char *term);
extern	int	termgraph_squares(pnode *p, int *number);
extern  int	brace_correct (char *bc);
extern	int 	set_flag_to_target (pnode *p);
extern	int	termgraph_window_main (Sgraph_proc_info info);
...
void 	call_termgraph_window_proc(Sgraph_proc_info info), call_term_to_graph_creation_proc(Sgraph_proc_info info),
	main_several_token_proc(Sgraph_proc_info info), termgraph_zoom_proc(Sgraph_proc_info info), termgraph_scroll_proc(Sgraph_proc_info info);
int 	main_proc_token(Sgraph_proc_info info), termgraph_fit_font(int h);
void 	termgraph_free_tree (pnode *p);	
int 	term_main(Sgraph_proc_info iinfo, int argc, char **argv);
int   	insert_edge_in_Sgraph_list(Snode snode, Snode tnode, int number1, int number2, int col, int eti, int efi, int elv, int al, float aa, Edgeline el, char *np, char *name_in);
Snode 	insert_node_in_Sgraph_list(Sgraph pgraph, pnode *p, int number, int nsx, int nsy, int nti, int nfi, int nlp, int nei, int nlv, int col, int npx, int npy, char *name_in);

Sgraph_proc_info  termglob_info;


void call_termgraph_window_proc(Sgraph_proc_info info)
{	
	termglob_info = info;
...
} /* call_termgraph_window_proc */

void petrinet_turn_proc (Sgraph pgraph, char start_from, int deltax, int deltay, int corx, int cory) 
   	                           ...
   	                           
{
	...
	int 	 do_it, move_x, move_y , sgnx, sgny, value;
...
void call_petrinet_turn_proc (Sgraph_proc_info info)
...
void call_term_to_graph_creation_proc(Sgraph_proc_info info)
...
void menu_termgraph_1 (Menu menu, Menu_item menu_item)
{
	call_sgraph_proc (call_termgraph_window_proc, NULL);
}

void menu_termgraph_2 (Menu menu, Menu_item menu_item)
{
...
int main_proc_token(Sgraph_proc_info info)
...
void main_several_token_proc(Sgraph_proc_info info)
...
void termgraph_zoom_proc(Sgraph_proc_info info)
...
void termgraph_scroll_proc(Sgraph_proc_info info)
{
	Snode 	 pnod;
	Sedge	 pedge;
	Edgeline el, edgeline;
