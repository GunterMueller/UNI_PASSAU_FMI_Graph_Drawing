...
**************************************************************************
**									**
**	ERSTELLUNG:	Rudolf Seisenberger, 	1994			**
**									**
...
**									**
**	extern call_term_to_graph_creation_proc ()				** 
...
#include "termgraph_export.h"
#include <xview/ttysw.h>

extern void termgraph_zoom_proc (Sgraph_proc_info info);
extern void termgraph_scroll_proc (Sgraph_proc_info info);
extern void term_main(Sgraph_proc_info iinfo, int argc, char **argv);
extern void call_petrinet_turn_proc(Sgraph_proc_info info), main_several_token_proc(Sgraph_proc_info info);
extern void main_proc_token(Sgraph_proc_info info);
...
static void ls_proc(void), select_file_proc(void), quit_proc(void), dirset_proc(void),
            show_proc (void), chdir_proc(void), string_proc(void), clear_proc(void), 
            termgraph_window_choice(Panel_item item, Event *event), termgraph_header_choice(Panel_item item, Event *event), termgraph_info_proc(void),
            load_agent_proc(void), /*quick_store_agent_proc(void),*/ store_agent_proc(void), draw_proc(Sgraph_proc_info info), 
            termgraph_call_zoom_proc(int info), termgraph_call_scroll_proc(Sgraph_proc_info info),
            /*token_proc(void), several_token_proc(void), */
            termgraph_done_proc(void), termgraph_set_misc_choice(Panel_item item, Event *event), termgraph_misc_set_proc(void),
            termgraph_set_basic_choice(Panel_item item, Event *event),
            termgraph_set_transx_choice(Panel_item item, Event *event), 
            termgraph_set_transy_choice(Panel_item item, Event *event), 
            termgraph_set_placex_choice(Panel_item item, Event *event),
            termgraph_set_placey_choice(Panel_item item, Event *event),
            termgraph_what_edgeline(Panel_item item, Event *event), turn_proc(Sgraph_proc_info info), cls_str(char *str),
            termgraph_load_values(void), termgraph_save_values(void);
char	    *termgraph_get_line(FILE *file);
static void create_panel_items (Sgraph_proc_info info);

int termgraph_window_main (Sgraph_proc_info info)
{
	...
	if (info->sgraph == empty_sgraph) {
		info->sgraph = make_graph (make_attr (ATTR_DATA, 0));	
	}
	if (info->sgraph->nodes == empty_snode) {
		Snode	node;
		node = make_node (info->sgraph, make_attr (ATTR_DATA, 0));
		node->x = node->y = 2000;
	}
...
static void ls_proc (void)
{
	char cmdstring[MAXBUF];

	sprintf (cmdstring, "ls %s | more\n", (char *)xv_get(termgraph_fname_item, PANEL_VALUE));
	ttysw_input (termgraph_tty, cmdstring, strlen (cmdstring));
	cls_str(cmdstring);
}

static void select_file_proc (void)
{
	char cmdstring[MAXBUF];
	char hname[MAXBUF];

	sprintf (hname, "%s", (char *)xv_get(termgraph_fname_item, PANEL_VALUE));
	if (hname[0] == '\0') {
		sprintf (cmdstring, 
...
static void dirset_proc (void)
{
	char cmdstring[MAXBUF];

	cls_str (termglob_agent_dir);

 	sprintf (termglob_agent_dir, "%s", (char *)xv_get(termgraph_fname_item, PANEL_VALUE));
 	sprintf (cmdstring, 
...
static void show_proc (void)
{
	...
	sprintf (cmdstring, 
		...
		(char *)xv_get(termgraph_fname_item, PANEL_VALUE), (char *)xv_get(termgraph_fname_item, PANEL_VALUE));
	if (cmdstring[13] == '\0')
...
static void chdir_proc (void)
{
	...
	if (strcmp ((char *)xv_get(termgraph_fname_item, PANEL_VALUE), "") != 0)
		sprintf (termglob_agent_dir, "%s/%s", termglob_agent_dir[0]=='\0'?".":termglob_agent_dir, (char *)xv_get(termgraph_fname_item, PANEL_VALUE));
...
static void termgraph_header_choice(Panel_item item, Event *event)
{
...
*/
		if (termglob_agent_name[0] != '\0') {
			...
			sprintf (str, "%s", (char *)xv_get(termgraph_header_item, PANEL_VALUE));
		}
		sprintf (filename, "%s", (char *)xv_get(termgraph_header_item, PANEL_VALUE));
...
static void load_agent_proc(void)
{
	char str[MAXNAME];
	FILE *file;
...
/*
static void quick_store_agent_proc(void)
{
	char str[MAXNAME];

	sprintf (str, "%s.agent-", (char *)xv_get(termgraph_header_item, PANEL_VALUE));
...
static void store_agent_proc(void)
{
	char str[MAXNAME];
...
static void termgraph_window_choice(Panel_item item, Event *event)
{
	switch ((int)xv_get(item, PANEL_VALUE))
	{
...
static void termgraph_set_misc_choice (Panel_item item, Event *event)
{	
...
static void termgraph_misc_set_proc(void)
{	
	char 	str[MAXNAME], button_str[MAXNAME];
	...
	sprintf ( button_str, "-");
	sprintf ( str, "%s", (char *)xv_get(termgraph_param_item1, PANEL_VALUE) );
	if ((str[0] == '\0') 
	...
	case 10: sprintf ( str, "%s", (char *)xv_get(termgraph_param_item1, PANEL_VALUE) );
		if (str[0] != '\0') {
			sscanf ( str, "%f", &fval );
...
static void termgraph_set_basic_choice(Panel_item item, Event *event)
{	
	char str[MAXNAME];
	int val;
	...
	case 4: termgraph_basic_length += 8;
		termgraph_only_defaults = 0;
		break;
	case 5: sprintf ( str, "%s", (char *)xv_get(termgraph_param_item1, PANEL_VALUE) );
		val = atoi (str);
		if (val > 0) {
...
static void termgraph_set_placex_choice(Panel_item item, Event *event)
{	
	char str[MAXNAME];
	int val;
	...
	case 5: termgraph_placex_length -= 2;
		termgraph_only_defaults = 0;
		break;
	case 6: sprintf ( str, "%s", (char *)xv_get(termgraph_param_item1, PANEL_VALUE) );
		val = atoi (str);
		if (val > 0) {
...
static void termgraph_set_placey_choice(Panel_item item, Event *event)
{	
	char str[MAXNAME];
	int val;
	...
	case 3: termgraph_placey_length = 3*termgraph_placex_length/4;
		termgraph_only_defaults = 0;
		break;
	case 4: sprintf ( str, "%s", (char *)xv_get(termgraph_param_item1, PANEL_VALUE) );
		val = atoi (str);
		if (val > 0) {
...
static void termgraph_set_transx_choice(Panel_item item, Event *event)
{	
	char str[MAXNAME];
	int val;
...
static void termgraph_set_transy_choice(Panel_item item, Event *event)
{	
	char str[MAXNAME];
	int val;
...
static void draw_proc (Sgraph_proc_info info)
{
	...
	call_sgraph_proc (call_term_to_graph_creation_proc, NULL);
...
/*
	xv_set(termgraph_frame,
		...
		NULL);
*/
	call_sgraph_proc (main_proc_token, NULL);
	xv_set(termgraph_header_item,
		PANEL_LABEL_X,		120,
...
static void several_token_proc (void)
{
...
/*
	xv_set(termgraph_frame,
		...
		NULL);
*/
	call_sgraph_proc (main_several_token_proc, NULL);
	xv_set(termgraph_header_item,
		PANEL_LABEL_X,		120,
...
static void termgraph_info_proc (void)
{
	xv_set(termgraph_header_item,
		PANEL_LABEL_X,		120,
		PANEL_LABEL_STRING, 	"- Termgraph - written by Rudolf Seisenberger - 1994 - Termgraph -            ",
		NULL);

}

static void termgraph_call_zoom_proc (int info)
{
	float dval;
	char str[MAXNAME];
/*
...
*/
				NULL);
			call_sgraph_proc (termgraph_zoom_proc, NULL);
			xv_set(termgraph_header_item,
				PANEL_LABEL_X,		120,
...
static void termgraph_load_values(void)
{
	FILE *file;
	...
	if ( (file = fopen(TERMGRAPH_PARAMETER_FILE, "r")) == NULL ) {
		sprintf (str, "NO %s", TERMGRAPH_PARAMETER_FILE);
	} else {
		...
		while ( line[0] != '\0') {
			...
			for (i=0; i<21; i++) {
				if ((strcmp(snam, name[i]) == 0) && (sval[0] != '\0')) {
					...
					default: sprintf (str, "UNKNOWN KEYWORD!");
						break;
					}
					break; /* for */
				}
...
static void termgraph_save_values(void)
{
	FILE *file;
	char str[MAXNAME];
...
static void termgraph_what_edgeline (Panel_item item, Event *event)
{
...
static void termgraph_call_scroll_proc (Sgraph_proc_info info)
{
/*
	xv_set(termgraph_frame,
		WIN_MOUSE_XY,	400+ (int)xv_get(termgraph_param_panel,WIN_X,0),
...
*/
	...
	call_sgraph_proc (termgraph_scroll_proc, NULL);
...
static void turn_proc(Sgraph_proc_info info)
{
/*
	xv_set(termgraph_frame,
		WIN_MOUSE_XY,	30+ (int)xv_get(termgraph_panel,WIN_X,0),
...
*/
	ttysw_input (termgraph_tty, "clear\n ", 7);
	call_sgraph_proc (call_petrinet_turn_proc, NULL);
