...
**									**
*************************************************************************/
extern Snode	insert_node_in_Sgraph_list(Sgraph pgraph, pnode *p, int number, int nsx, int nsy, int nti, int nfi, int nlp, int nei, int nlv, int col, int npx, int npy, char *name_in);
extern int	insert_edge_in_Sgraph_list(Snode snode, Snode tnode, int number1, int number2, int col, int eti, int efi, int elv, int al, float aa, Edgeline el, char *np, char *name_in);
extern int	add_to_y(pnode *p, int delta), add_to_x(pnode *p, int delta);
extern int 	sema_name_char(char c);
extern int 	max_x_in_tree(pnode *p, char *stop_here);
extern int 	min_x_in_tree(pnode *p, char *stop_here);
extern pnode	*find_critical(pnode *p, char *x);
extern int	the_two_criticals(pnode *synchron_node, char *name1, char *name2);
extern int 	make_sema_construction (Sgraph pgraph, pnode *synchron_node);
...
pnode		*(next_edge_to_op(char op, pnode *p));
int		lift_lower_node_y (pnode *node);
pnode		*createpnode(void);
Edgeline 	(make_rec_itoa_string(pnode *hlp_out, pnode *p, char *np));
Snode		make_node_and_edge_for_parallel_places(Sgraph pgraph, int *number, int nsx, int nsy, int nti, int nfi, int nlp, int nei, int nlv, int ncol, int ecol, int npx, int npy, int eti, int efi, int elv, int al, float aa, pnode *p, pnode *psuc);
Edgeline 	termgraph_make_edgeline_angle(char mode, int px, int py, int dx, int dy, int qx, int qy);
int		operation_sema_edges(Sgraph pgraph, pnode *p, int *num, int nsy, int nsx, int nti, int nfi, int nlp, int nei, int nlv, int ncol, int ecol, int npx, int npy, int eti, int efi, int elv, int al, float aa, Edgeline eline, char *np);
int		operation_or_edges(Sgraph pgraph, pnode *p, int nsy, int nsx, int nti, int nfi, int nlp, int nei, int nlv, int ecol, int npx, int npy, int eti, int efi, int elv, int al, float aa, int nx, int ny, Edgeline el, char *np);
int		operation_parallel_construct(Sgraph pgraph, pnode *p, char op, int nsy, int nsx, int nti, int nfi, int nlp, int nei, int nlv, int ecol, int npx, int npy, int eti, int efi, int elv, int al, float aa, int nx, int ny, Edgeline eline, char *np);
int		graph_edges_top_down(Sgraph pgraph, pnode *p_input, int *num);
int		graph_nodes_top_down(Sgraph pgraph, pnode *p_input, int *num);
int 		recursion_under_construction (pnode *p, pnode *f);
char		*init_graph_file();
pnode		*next_edge_to_op(char op, pnode *p);
int 		graph_synchron_top_down(Sgraph pgraph, pnode *p);
...
int graph_edges_top_down(Sgraph pgraph, pnode *p_input, int *num)
{
	...
	float		aa;
	int		nx, ny, x1;
	char 		np[MAXBUF]; /*willkuerliche Obergrenze*/
	pnode		*hlp, *psuc, *p;
...
int operation_parallel_construct (Sgraph pgraph, pnode *p, char op, int nsy, int nsx, int nti, int nfi, int nlp, int nei, int nlv, int ecol, int npx, int npy, int eti, int efi, int elv, int al, float aa, int nx, int ny, Edgeline eline, char *np)
     		            ...
     		            /*willkuerliche Obergrenzen*/
	
{
	int 		i, lactiv, ractiv, *num=&(p->cardinal), insert;
	pnode		*hlp;
	Snode		num_Snod, num2_Snod;
	Edgeline	el;
...
{
	int 		lactiv, ractiv, nx, ny, insert;
	pnode		*hlp;
	Snode		num_Snod, num2_Snod;
	Edgeline	el;
...
int  check_local_successor_activity(char op, int pnumber, int px, int py, pnode *psuc)
...
int termgraph_draw_petrinet(Sgraph pgraph, FILE *filep, pnode *p, int *num, int argc, char **argv, char *term)
{
