...
char	*(fill_buffer(Sgraph pgraph, char *input_name, char **output_string));
char 	*flatten(char *ch_in);
int	brace_syntax_correct(char leftc, char rightc, char *str, int *fault);
int	brace_correct(char *bc);
char 	*(call_name(int argc, char **argv));
char 	*substitutions(char *ch_in);
...
{		
	...
	static char	flats[MAXBUF];
	char		ch[MAXBUF], term[MAXBUF];
	int		input_error;
	    ...
	    ***/
	    ...
	    case '|':	/*** 	tolerante Analyse:
	    		...
	    		***/
	    		...
	    		if (( ((ch[i+1]=='$' || ch[i+1]=='$') && ((ch[i+2]!=0)&&(ch[i+2]=='{')) )
	    			/*** zweimal '$' durch z. B. 's' oder 'S' ersetzbar ***/
	    			|| ch[i+1] =='$')     
	    			&& ((ch[i+2]!=0) && (ch[i+2]==' ' || ch[i+2]=='{'))) {
	    			/* ch[i] = ' ';
	    			ch[i+1] = '$'; redundant! */
...
{
	static char ch[MAXBUF];
	char subst[MAXBUF], ch_tmp[MAXBUF], cmpname[MAXBUF], 
	     copystring[MAXBUF], subst_rest[MAXBUF];
	int  anf, drin, u, j, k, l, m, copystring_len, cmpname_len;
	int  with_name;
	...
	drin = k = 0;
	while ( (anf < MAXBUF) &&  (subst[anf] != '\0') && (anf < (int)strlen(subst) ) && (subst[anf] != TERM_END_CHR) ) {
