...
static Slist FindSepPair(Snode NodeA, Snode NodeB, Slist SepPairList)
...
static void AddToSepPairList(Snode FirstNode, Snode SecondNode, Slist *SepPairList)
...
static void AddToVirtualEdgeList(Sedge TheEdge, Slist SepPair)
...
static void LinkVirtualEdges(Sedge First, Sedge Second)
...
static Slist MakeVirtualEdge(Snode SNode, Snode TNode, int Nr)
...
static void AddToNewComp(Slist NewEdge, Slist *SplitComp)
...
static void AddToSplitCompList(Slist SplitComp, SplitCompType CompType, Slist *SplitList)
...
static void  PushTStack(TripleStack *TheTStack, Snode HNode, Snode ANode, Snode BNode)
...
static void FindSplitComp(Sgraph TheGraph, Snode CurrentNode, TripleStack *TStack, EdgeStack *EStack, int *FLAG, int *j, Slist *SplitList, Slist *SepPairList)
                     
                        
                         /* Zur Definition von 'TripleStack' siehe FindSplitComp.h. */
                         /* Kantenstack (Definition siehe Utilities.h) */
                  
               
                            /* Liste der Splitkomponenten */
                              /* Liste der Spaltungspaare */
...
/*******************************************************************************/
...

  {
    ...
    TripleStack  LastElem;
    Snode        x,y = EmptySnode,TargetNode;
    bool         deleted = FALSE;
    EdgeStack    LastEdge;
                ...
                if (FirstEdgeOfPath(EdgePointer(ActElem)))
                        {
                          ...
                          if (!deleted)
                             ...
                             else  PushTStack(TStack,NMAX(y,FindNode(TheGraph,NEWNUM,NewNum(TargetNode)+Nd(TargetNode)-1)),LowPt1(TargetNode),BSepNode(LastElem));
...
void DetermineSplitComponents(Sgraph TheGraph, Slist *SplitList, Slist *SepPairList)
{
