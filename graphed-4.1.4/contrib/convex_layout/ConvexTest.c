...
static void BuildComp(Slist ActElem)
...
static void FindThreeConnectedComp(Slist TheGraphList)
...
static bool ContainsRealEdge(Slist TheComp)
...
static void DeterminePrimeSepPairs(Slist SepPairsListElem)
...
static void RemoveRing(SedgeStack TheRing)
...
static void AddToRing(Slist SplitComp, SedgeStack *TheRing)
  /* Aufbau eines Ringes */
...
static void CheckTyp(Slist SepPairsElem, Sedge VirtEdge, int *counter3con, int *countertriangle, int *counterbond, int *OneRealEdge, SedgeStack *TheRing, int *RingFound)
...
static void DetermineTypOfPrime(Slist SepPairsListElem)
...
static Slist CheckSepPairs(Slist SepPairsList, Slist SplitList, int *NumberOfCriticalPairs)
...
static void InitEmbededNodeEdgeAttrs(Sgraph TheGraph)
    ...
    for_all_nodes(TheGraph,ActNode)
       ...
       for_slist(OrderedList,ActElem)
            ...
            if (EdgeEntry(EmbededEdge(ActElem)) == empty_slist)
                     ...
                     free(EmbededEdge(ActElem)->attrs.value.data);
                   }
...
static bool FindPath(Slist *ThePath, Slist TheEdge, Snode TNode)
{
...
static void DeleteOneXYPath(Snode SNode, Snode TNode)
...
static bool IsEdgeThenDelete(Snode SNode, Snode TNode)
...
static void DeleteXYPath(SedgeStack TheRing)
...
static Slist FindFacialCycleWithXY(Snode x, Snode y)
...
static Slist FacialCycleOfSevenPossibleGraphs(Sgraph TheGraph, Slist SepPairList)
...
static Sedge CopySedge(Sedge TheEdge)
...
static bool IsEdgeThenRealyDelete(Sgraph TheGraph, Snode SNode, Snode TNode, Slist *DeletedEdges)
...
static Snode CopySnode(Snode TheNode)
...
static void DeleteXYPathRealy(Slist SepPair, SedgeStack TheRing, Slist *DeletedEdges)
...
static Slist ComputeGraphG1(Sgraph TheGraph, Slist SepPairList)
...
static void ComputeGraphG2(Sgraph TheGraph, Slist SepPairList, Slist *DeletedEdges, Snode *NewNode)
...
static void CopyNodeEntries(Snode SNode, Snode TNode)
{
...
static void CopyEdgeEntries(Sedge SEdge, Sedge TEdge)
{
...
static void RecoverOldGraph(Sgraph TheGraph, Slist DeletedEdges, Snode NewNode)
    ...
    for_slist(DeletedEdges,ActElem)
       ...
       if (SavedEdgesList(ActElem) == empty_slist)
              {            
                ...
                free(SavedEdge(ActElem)->attrs.value.data);
                ...
                for_slist(SavedEdgesList(ActElem),ActEdge)
                            ...
                            if (LastNode == empty_snode)
                                   {
                                     ...
                                     free(Target(ActEdge)->attrs.value.data);
                                    ...
                                    if (LastNode == empty_snode)
                                           {
                                             ...
                                             free(Source(ActEdge)->attrs.value.data);
                   ...
                   free(LSavedEdge(ActEdge)->attrs.value.data);
...
static SedgeStack FindVCycle(Snode V)
...
static Slist MakeExtendibleFacialCycleFromVCycle(SedgeStack TheVCycle)
    ...
    if ((TheVCycle->edge->snode == TheVCycle->suc->edge->snode) || (TheVCycle->edge->snode == TheVCycle->suc->edge->tnode))
           {
             ...
             for_slist(OrderedEdgeList(TheVCycle->edge->tnode),ActEdge)
                if (TargetNodeGet(ActEdge) == TNode)
                       {
                         TheExtCycle = new_slist(make_attr(ATTR_DATA,(char *)ActEdge->attrs.value.data));
                         break; 
                       }    
         ...
         if (TNode == TheVCycle->edge->snode)
                {
                  for_slist(OrderedEdgeList(TNode),ActEdge)
                     if (TargetNodeGet(ActEdge) == TheVCycle->edge->tnode)
                            {
                              add_immediately_to_slist(TheExtCycle,make_attr(ATTR_DATA,(char *)ActEdge->attrs.value.data));
                              break;   
                            }
                  end_for_slist(OrderedEdgeList(TNode),ActEdge);
                  TNode = TheVCycle->edge->tnode;
...
static void RemoveSepPairList(Slist TheSepPairList)
...
static void RemoveSplitCompList(Slist TheSplitCompList)
...
static void RemoveOrderedEdgeList(Snode TheNode)
{
    Slist  ActEdge;
...
void RemoveOrderedNEAttrs(Sgraph TheGraph)
...
Slist  ConvexityTest(Sgraph TheGraph)
