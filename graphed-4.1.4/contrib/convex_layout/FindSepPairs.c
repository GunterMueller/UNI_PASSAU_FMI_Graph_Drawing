...
static void InitEdgeAttrs(Sgraph TheGraph)
...
static void InitNodeAttrs(Sgraph TheGraph)
...
static void RemoveEdgeAttrs(Sedge TheEdge)
...
static void RemoveAdjacencyList(Slist TheSlist)
...
static void RemoveNodeAttrs(Snode TheNode)
...
void RemoveSplitAttrs(Sgraph TheGraph)
{
    Snode  ActNode;
...
static void  MarkPtreeEdge(Slist TheSlistElem, EdgeType TheEdgeType)
...
static void  DFS(int *n, Snode SonNode, Snode FatherNode)
...
/*  Das verwendete Makro 'NMIN' ist im zugehoerigen Header-File definiert und   */
/*  dokumentiert.                                                              */
/*******************************************************************************/
       ...
       if (Number(TargetNode) == 0)
              {
                ...
                if (Number(LowPt1(TargetNode)) < Number(LowPt1(SonNode)))
                       {
                         LowPt2(SonNode) = NMIN(LowPt1(SonNode),LowPt2(TargetNode));
                         LowPt1(SonNode) = LowPt1(TargetNode);
                       }/*then*/
                   else if (Number(LowPt1(TargetNode)) == Number(LowPt1(SonNode)))
                                 LowPt2(SonNode) = NMIN(LowPt2(SonNode),LowPt2(TargetNode));
                           else  LowPt2(SonNode) = NMIN(LowPt2(SonNode),LowPt1(TargetNode));
                         ...
                         if (Number(TargetNode) < Number(LowPt1(SonNode)))
                                {
                                  ...
                                  if (Number(TargetNode) > Number(LowPt1(SonNode)))
                                           LowPt2(SonNode) = NMIN(LowPt2(SonNode),TargetNode);
                                }/*else*/
                       }/*then*/
              }/*else*/
...
static void DeleteDoubleEdges(Sgraph TheGraph)
       ...
       while(EStack != EmptyStack)
         {
           ...
           free((EStack->edgelistelem)->attrs.value.data);   /* Freigeben des Speichers fuer die Listenattribute. */
           EStack = EStack->suc;
         }/*while*/
    end_for_all_nodes(TheGraph,ActNode);
...
static int phi(Slist TheSlistElem)
...
static void BuildAdjacencyLists(Sgraph TheGraph, int V)
                
                     /* Anzahl der Knoten von TheGraph */
...
static void PathFinder(Sgraph TheGraph, Snode FirstNode, Snode *Ess, int *m)
...
static void DetermineDegreeOfNodes(Sgraph TheGraph)
...
static void FindFirstSonOfNodes(Sgraph TheGraph)
...
void FindSeperationPairs(Sgraph TheGraph)
{
     int n = 0;
