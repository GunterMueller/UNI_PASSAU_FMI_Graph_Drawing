...
/*1994-91 by Stefan Jockenhoevel and Gerd Nienhaus.*/
...
static	void	DrawEntryConvex(Slist FacialCycle, Point v, Snode v1, Snode v2, double weight, int numberofnodes);
...
static void CheckEdgeline(Edgeline TheEdgeLine)
...
static void DrawFacialCycleConvex(Sgraph TheGraph, Slist FacialCycle, int edit)
...
static double ComputeAreaOfTriangle(Snode M, Snode A, Snode B)
...
static double ComputeAreaOfBlock(Snode V, Snode FirstNode, Slist TheCycle)
...
static double ComputeWeight(double areaofblock, double weightoffaces, Slist TheCycle, Snode V, Snode LastNode, int apexcounter)
{
    Snode     V1;
    Slist     ActEdge;
    int       i;
...
static Point Intersection(Point b1, Point e1, Point b2, Point e2)
...
static Slist New_v1_v2(Slist FacialCycle, Snode v1, Snode v2, Snode *new_v1, Snode *new_v2) 
...
static void DrawUnconnectedNodes(Slist FacialCycle, Snode v1, Snode v2)
...
static void DrawEntryConvex(Slist FacialCycle, Point v, Snode v1, Snode v2, double weight, int numberofnodes)
  ...
  /* ist numberofnodes = 0, so wird NICHTS gemacht */
                           ...
                           /* Hoehe der Verschiebung */
                           /* Anzahl der Knoten, die mit v verbunden sind */
...
static void EleminateNodesOfDegree2(Sgraph TheGraph)
...
static void DrawDegree2Node(Snode TheNode)
{
    Point     A,B;
...
static void DrawNodesOfDegree2(Sgraph TheGraph)
...
static Slist FindBlockCyclePre(Slist FirstEdge, Slist *LastEdge, int nr, int *apexcounter)
{
...
static Slist FindBlockCycleSuc(Slist FirstEdge, Slist *LastEdge, int nr, int *apexcounter)
{
    Slist  FacialEdge,TheBlockCycle = empty_slist;
...
static void SearchNodesAndEdges(Slist FirstEdge, int *nodecounter, int *edgecounter, int nr)
{
    Slist   ActElem;
...
static int CountFacesOfBlockSuc(Slist TheCycle, int nr)
...
static int CountFacesOfBlockPre(Slist TheCycle, int nr)
...
static bool NodesAreKollinear(Snode A, Snode B, Snode C)
...
static void ExtendFacialCyclePre(Slist TheFacialCycle, int nr)
{
    Snode     StartNode,TargetNode;
    Slist     TheCycle,ActEdge,LastEdge,Start;
    int       apexcounter,numberoffacesinblock;
    void      ExtendFacialCycleSuc(Slist TheFacialCycle, int nr);
    Point     v;
...
void ExtendFacialCycle(Sgraph TheGraph, Slist TheFacialCycle, int edit)
