...
{
	struct fpp_node *gra;
	struct nlist *hneigh1;
	struct fpp_node  *h_row;
	double b, newpoint;
	int  sec, sec2;
	Snode n;
...

{
	...
	lastgra = (struct fpp_node *) malloc (sizeof (struct fpp_node));
	lgraph = lastgra;
	...
	while (hgraph != NULL) {
		...
		if (hgraph->next != NULL) {
			lgraph->next = (struct fpp_node *) malloc (sizeof (struct fpp_node));
			lgraph = lgraph->next;
		}
		else {
			lgraph->next = NULL;
