...
void set_kind_of_nodes (struct polygon *poly)
...
{
	struct fpp_node *p, *pn0, *pn1, *pn2, *local, *maxele, *pri;
	int is_sort, loop;
...
void sort_y_coordinates_2 (struct polygon *p)
...
{
	struct location_edge *ll;
	struct fpp_node *dummy1, *dummy2;

	location_list = (struct location_edge *) malloc (sizeof (struct location_edge));
	ll = location_list;
	ll->rel_node = n;
	dummy1 = (struct fpp_node *) malloc (sizeof (struct fpp_node));
	dummy2 = (struct fpp_node *) malloc (sizeof (struct fpp_node));
	dummy1->number = -1;
	dummy2->number = -2;
	dummy1->x = -1;
	dummy1->y = 10;
